(function (global, factory) {
  ...
  function buildHighlightedText(text, positions, snippetLength) {
    ...
    const validPositions = positions.filter(
      (position) =>
        position.length > 0 && position.start + position.length <= textLength
    );
      ...
      return [
        {
          ...
          text:
            text.slice(
              0,
              snippetLength >= textLength ? textLength : snippetLength
            ) + (snippetLength < textLength ? '...' : ''),
        },
      ];
    }

    const orderedPositions = validPositions.sort(
    ...
    if (snippetLength && text.length > snippetLength) {
      ...
      range.start =
        firstPositionStart - snippetLength < 0
          ? 0
          : firstPositionStart - snippetLength;
      range.end =
        firstPositionEnd + snippetLength > textLength
          ? textLength
          : firstPositionEnd + snippetLength;
    }
    const nodes = [];
    if (firstPosition.start > 0) {
      nodes.push({
        ...
        text:
          (range.start > 0 ? '...' : '') +
          text.slice(range.start, firstPosition.start),
      });
    }
    ...
    const positionsWithinRange = orderedPositions.filter(
      (position) =>
        ...
        position.start + position.length <= range.end
    );
    ...
    if (lastEndPosition < range.end) {
      nodes.push({
        type: 'text',
        text:
          text.slice(lastEndPosition, range.end) +
  ...
  const facetFilterInput = document.querySelector(
    '#search-field input[type=checkbox][data-facet-filter]'
  );
  ...
  function createSearchResult(result, store, searchResultDataset) {
    ...
    result.forEach(function (item) {
      ...
      const componentVersion =
        store.componentVersions[`${doc.component}/${doc.version}`];
      if (
      ...
      searchResultDataset.appendChild(
        createSearchResultItem(doc, sectionTitle, item, highlightingResult)
      );
    });
  }
      ...
      index.query(function (lunrQuery) {
        lunrQuery.clauses = query.clauses.map((clause) => {
          if (clause.presence !== globalThis.lunr.Query.presence.PROHIBITED) {
            clause.term = '*' + clause.term + '*';
            clause.wildcard =
              globalThis.lunr.Query.wildcard.LEADING |
              globalThis.lunr.Query.wildcard.TRAILING;
            clause.usePipeline = false;
          }
          return clause;
        });
  ...
  function executeSearch(index) {
    const debug =
      'URLSearchParams' in globalThis &&
      new URLSearchParams(globalThis.location.search).has('lunr-debug');
  ...
  function initSearch(lunr, data) {
    ...
    if (facetFilterInput) {
      ...
      facetFilterInput.addEventListener('change', (e) =>
