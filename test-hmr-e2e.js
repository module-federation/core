...
const runHMRTest = async () => {
  try {
    ...
    console.log(
      `Hash changed: ${initialHash !== newHash ? '✅ YES' : '❌ NO'}`,
    );
    ...
    console.log(
      '   - "[Module Federation Debug]" messages showing hash comparisons',
    );
    console.log(
      '   - "🔥 SERVER-SIDE REMOTE CHANGE DETECTED" if change was detected',
    );
    ...
    console.log(
      '   - Currently HMR is skipped on server-side, but detection should work',
    );
  } catch (error) {
