...
function getExistingCacheEntry(url, kind, nextUrl, prefetchCache, allowAliasing) {
    if (kind === void 0) kind = PrefetchKind.TEMPORARY;
    ...
    for (const maybeNextUrl of [
        ...
        if (process.env.NODE_ENV !== 'development' && allowAliasing && url.search && kind !== PrefetchKind.FULL && entryWithoutParams && // We shouldn't return the aliased entry if it was relocated to a new cache key.
    ...
    if (process.env.NODE_ENV !== 'development' && kind !== PrefetchKind.FULL && allowAliasing) {
 ...
 */ export function createSeededPrefetchCacheEntry(param) {
 ...
 */ function createLazyPrefetchEntry(param) {
    ...
    const data = prefetchQueue.enqueue(()=>fetchServerResponse(url, {
...
export const STATIC_STALETIME_MS = Number(process.env.__NEXT_CLIENT_ROUTER_STATIC_STALETIME) * 1000;
function getPrefetchEntryCacheStatus(param) {
    ...
    if (staleTime !== -1) {
        ...
        return Date.now() < prefetchTime + staleTime ? PrefetchCacheEntryStatus.fresh : PrefetchCacheEntryStatus.stale;
    }
