...
if (__webpack_require__.f) {
  const originalReadFile = false;
  const originalRequire = false;
  if (originalReadFile) {
  ...
  if (originalRequire) {
    __webpack_require__.f.require = function (chunkId, promises) {
      ...
      const fs = require('fs');
      if (fs && !fs.existsSync(filename)) return;
      originalRequire(chunkId, promises);
    };
  }
  // Dynamic filesystem chunk loading for javascript
  __webpack_require__.f.readFileVm() = function (chunkId, promises) {
    let installedChunkData = installedChunks[chunkId];
    console.log(installedChunks);
    ...
    if (__webpack_require__.federation.bundlerRuntimeOptions.consumes) {
      if (
        __webpack_require__.federation.bundlerRuntimeOptions.consumes
          .chunkMapping[chunkId]
      )
        return;
    }
