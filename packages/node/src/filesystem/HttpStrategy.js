...
class HttpStrategy {
  async loadChunk(chunkId,rootOutputDir, remotes, callback) {
    ...
    https.get(scriptUrl.toString(), (res) => {
      let content = '';
      ...
      res.on('end', () => {
        ...
        try {
          vm.runInThisContext(`(function(exports, require, __dirname, __filename) {${content}\n})`, scriptUrl.toString())(
            chunk, require, require('path').dirname(scriptUrl.pathname), scriptUrl.pathname
          );
          callback(null, chunk);
        } catch (e) {
      ...
      logger(`'error loading remote chunk'`, scriptUrl.toString(), 'got', err);
      callback(err, null);
    });
  }
}
