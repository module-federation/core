...
it.each([undefined, 'common'])(
  'emits concrete provider alternatives with layer %s',
  async (layer) => {
    ...
    try {
      ...
      const compiler = webpack({
        ...
        entry: { first: './shared.js', second: './shared.js' },
        optimization: { runtimeChunk: 'multiple' },
        output: { path: directory, publicPath: '/', filename: '[name].js' },
      ...
      expect(artifact.shared).toHaveLength(layer ? 2 : 1);
      const row = artifact.shared.find((item) => item.layer === layer);
      ...
      expect(row.providers).toEqual([
        {
          ...
          assets: {
            js: {
              sync: [
                layer ? '_common_shared_js_first.js' : 'shared_js_first.js',
              ],
              async: [],
            },
            css: { sync: [], async: [] },
      ...
      if (layer)
        expect(
          artifact.shared.find((item) => item.layer === 'client').providers,
        ).toBeUndefined();
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
