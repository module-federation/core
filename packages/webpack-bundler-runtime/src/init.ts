...
import type {ModuleFederationRuntimePlugin} from '@module-federation/runtime'
 ...
 const buildTreeShakeSharePlugin: ({webpackRequire}:{webpackRequire: WebpackRequire}) => ModuleFederationRuntimePlugin =
  function ({webpackRequire}:{webpackRequire: WebpackRequire}) {
    return {
      name: 'build-tree-shake-plugin',
      beforeInit(args){
       const {fallbackSharedAssets,usedExports,runtime,bundlerRuntime} = webpackRequire.federation;
        ...
        [...Array.isArray(shareArgs) ? shareArgs :[shareArgs]].forEach(shareConfig=>{
          ...
          const shareUsedExports = webpackRequire.j ? usedExports?.[shareName][webpackRequire.j] : undefined
          if(shareUsedExports){
...
export function init({webpackRequire}:{webpackRequire: WebpackRequire}){
    const {initOptions,runtime,fallbackSharedAssets,usedExports} = webpackRequire.federation;
