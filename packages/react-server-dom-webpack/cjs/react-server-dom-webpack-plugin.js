...
class ReactFlightWebpackPlugin {
  constructor(options) {
    ...
    this.clientManifestFilename =
      options.clientManifestFilename === false ||
      options.clientManifestFilename === null
        ? null
        : options.clientManifestFilename || 'react-client-manifest.json';
    this.serverConsumerManifestFilename =
      options.serverConsumerManifestFilename === false ||
      options.serverConsumerManifestFilename === null
        ? null
  ...
  applyServer(compiler) {
    ...
    function matchesLayer(mod, fallbackLayer) {
      if (!_this.layers || _this.layers.length === 0) return true;
      const layer =
        (mod &&
          (mod.layer ||
            (typeof mod.getLayer === 'function'
              ? mod.getLayer()
              : undefined))) ||
        fallbackLayer;
  ...
  applyClient(compiler) {
    ...
    compiler.hooks.make.tap('ReactFlightPlugin', (compilation) => {
      compilation.hooks.processAssets.tap(
        {
          name: 'ReactFlightPlugin',
          // Use SUMMARIZE so react-client-manifest.json is available for MF additionalData.
        ...
        function () {
          if (!1 === clientFileNameFound)
            compilation.warnings.push(
              new webpack.WebpackError(
                'Client runtime at @module-federation/react-server-dom-webpack/client was not found. React Server Components module map file ' +
            ...
            compilation.chunkGroups.forEach(function (chunkGroup) {
              function recordModule(id, module) {
                ...
                function visit(mod, parentLayer) {
                  if (!mod) return;
                  if (!matchesLayer(mod, parentLayer)) return;
                  const isClient =
                    mod.buildInfo &&
                  ...
                  if (mod.modules && Array.isArray(mod.modules)) {
                    const nextLayer =
                      ...
                      (typeof mod.getLayer === 'function'
                        ...
                        : undefined) ||
                      parentLayer;
                    mod.modules.forEach((child) => visit(child, nextLayer));
                  }
                }

                visit(module, module.layer);
              }
              const chunks = [];
            ...
            if (_this.clientManifestFilename) {
            ...
            if (_this.serverConsumerManifestFilename) {
