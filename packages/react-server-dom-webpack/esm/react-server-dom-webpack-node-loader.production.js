...
function transformServerModule(source, program, url, sourceMap, loader) {
  ...
  if (exportedEntries.length > 0) {
    ...
    newSrc += '\n\n;';
    newSrc +=
      'import {registerServerReference} from "@module-federation/react-server-dom-webpack/server";\n';
    if (mappings) {
      mappings += ';;';
    }
...
async function transformClientModule(program, url, sourceMap, loader) {
  ...
  let newSrc =
    'import {registerClientReference} from "@module-federation/react-server-dom-webpack/server";\n';
