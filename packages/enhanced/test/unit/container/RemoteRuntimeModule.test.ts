...
describe('RemoteRuntimeModule', () => {
  ...
  describe('generate', () => {
    it('should return null when no remote modules are found', () => {
      ...
      expect(result).toContain(
        '__webpack_require__.e.remotes = function(chunkId, promises) { __FEDERATION__.bundlerRuntime.remotes({ chunkId, promises, webpackRequire:__webpack_require__}); }',
      );
    });
    ...
    it('should process remote modules and generate correct runtime code', () => {
      ...
      // Verify the remotesLoadingData.chunkMapping
      expect(result).toContain(
        '__webpack_require__.remotesLoadingData.chunkMapping',
      );
      ...
      // Verify the remotesLoadingData.moduleIdToRemoteDataMapping
      expect(result).toContain(
        '__webpack_require__.remotesLoadingData.moduleIdToRemoteDataMapping',
      );

      expect(result).toContain('"module1": {');
      expect(result).toContain('"shareScope"');
      expect(result).toContain('"default"');
      expect(result).toContain('"name"');
      expect(result).toContain('"./component1"');
      expect(result).toContain('"external1"');
      expect(result).toContain('"remoteName"');
