...
it('should load tree shaking shared via set infer strategy', async () => {
  ...
  const uiLibShared =
    __FEDERATION__.__SHARE__['tree_shaking_share:0.0.0'].default['ui-lib'][
      '1.0.0'
    ];
  expect(uiLibShared.loaded).toEqual(undefined);
  expect(uiLibShared.treeShaking.loaded).toEqual(true);
  ...
  const uiLibESShared =
    __FEDERATION__.__SHARE__['tree_shaking_share:0.0.0'].default['ui-lib-es'][
      '1.0.0'
    ];
  expect(uiLibESShared.loaded).toEqual(undefined);
  expect(uiLibESShared.treeShaking.loaded).toEqual(true);

  expect(Object.keys(uiLibESShared.treeShaking.lib()).sort()).toEqual([
    'Button',
  ]);
...
it('should not tree shaking ui-lib-dynamic-default-export', async () => {
