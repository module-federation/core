import { ModuleFederationPlugin, dependencies } from '@module-federation/enhanced';
...
describe('HoistContainerReferencesPlugin', () => {
    ...
    compiler.run((err, stats) => {
      try {
        ...
        if (stats.hasErrors()) {
          ...
          const info = stats.toJson({ errorDetails: true, all: false, errors: true });
          console.error("Webpack Errors:", JSON.stringify(info.errors, null, 2));
          return done(new Error(info.errors?.map(e => e.message + (e.details ? `\n${e.details}` : '')).join('\n')));
        }
        if (stats.hasWarnings()) {
             console.warn("Webpack Warnings:", stats.toString({ colors: true, all: false, warnings: true }));
        }
        ...
        const runtimeChunk = Array.from(compilation.chunks).find((c: Chunk) => c.hasRuntime() && c.name === 'runtime');
        ...
        for(const module of compilation.modules) {
          ...
          if (module.dependencies?.some(dep => dep instanceof FederationRuntimeDependency)) {
            federationRuntimeOriginModule = module;
            break;
          }
          if (module.blocks?.some(block => block.dependencies?.some(dep => dep instanceof FederationRuntimeDependency))) {
            federationRuntimeOriginModule = module;
            break;
          }
        }
        expect(federationRuntimeOriginModule).toBeDefined();
        if(!federationRuntimeOriginModule) return done(new Error('Module originating FederationRuntimeDependency not found'));

        // 3. Assert the Federation Runtime Origin Module is in the Runtime Chunk
        const isRuntimeOriginInRuntime = chunkGraph.isModuleInChunk(federationRuntimeOriginModule, runtimeChunk);
        ...
        const mainChunk = Array.from(compilation.chunks).find((c: Chunk) => c.name === 'main');
        if (mainChunk && mainChunk !== runtimeChunk) {
          const isRuntimeOriginInMain = chunkGraph.isModuleInChunk(federationRuntimeOriginModule, mainChunk);
  ...
  it('should NOT hoist container entry but hoist its deps when using exposes', (done) => {
    ...
    const packageJsonContent = '{ "name": "test-host-exposes", "version": "1.0.0" }';
    ...
    compiler.run((err, stats) => {
      try {
        ...
        for(const module of compilation.modules) {
          if(module.dependencies.some(dep => dep instanceof dependencies.ContainerEntryDependency)) {
             containerEntryModule = module;
             break;
          }
        }
        expect(containerEntryModule).toBeDefined();
        if(!containerEntryModule) return done(new Error('ContainerEntryModule not found via dependency check'));

        // 3. Find the exposed module itself
        const exposedModule = Array.from(compilation.modules).find(m => m.identifier().endsWith('exposed.js'));
        expect(exposedModule).toBeDefined();
        if(!exposedModule) return done(new Error('Exposed module (exposed.js) not found'));

        // 4. Get all modules referenced by the container entry
        const referencedModules = getAllReferencedModules(compilation, containerEntryModule, 'all');
        ...
        const isContainerInRuntime = chunkGraph.isModuleInChunk(containerEntryModule, runtimeChunk);
        expect(isContainerInRuntime).toBe(false);

        // 6. Assert the exposed module is NOT in the runtime chunk
        const isExposedInRuntime = chunkGraph.isModuleInChunk(exposedModule, runtimeChunk);
        ...
        for (const module of referencedModules) {
            ...
            if (module === containerEntryModule || module === exposedModule) continue;

            const isModuleInRuntime = chunkGraph.isModuleInChunk(module, runtimeChunk);
