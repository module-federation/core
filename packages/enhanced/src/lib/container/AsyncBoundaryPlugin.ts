...
class AsyncEntryStartupPlugin {
  ...
  private handleRenderStartup(compiler: Compiler, compilation: Compilation): void {
    compiler.webpack.javascript.JavascriptModulesPlugin.getCompilationHooks(compilation).renderStartup.tap('AsyncEntryStartupPlugin', (source: any, renderContext: Module, upperContext: { chunk: Chunk }) => {
      ...
      for (const runtimeItem of runtime) {
        const requirements = compilation.chunkGraph.getTreeRuntimeRequirements(runtimeItem as Chunk);
