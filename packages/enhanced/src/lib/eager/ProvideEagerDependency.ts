...
class ProvideEagerDependency extends Dependency {
  shareScope: string;
  name: string;
  ...
  override getResourceIdentifier(): string | null {
    return `provide eager (${this.shareScope}) ${this.request} as ${
      this.name
   ...
   * @returns {ProvideEagerDependency} deserialize fallback dependency
   */
  static deserialize(
    context: ObjectDeserializerContext,
  ): ProvideEagerDependency {
    const { read } = context;
    const obj = new ProvideEagerDependency(
      read(),
      read(),
      read(),
      read(),
...
makeSerializable(
  ProvideEagerDependency,
  'enhanced/lib/sharing/ProvideEagerDependency',
);

export default ProvideEagerDependency;
