...
import ProvideForEagerDependency from './ProvideForEagerDependency';
import { WEBPACK_MODULE_TYPE_EAGER } from '../Constants';
...
const TYPES = new Set(['eager-init']);

/**
 * @class
 * @extends {Module}
 */
class ProvideEagerModule extends Module {
  ...
  constructor(
    ...
    super(WEBPACK_MODULE_TYPE_EAGER);
  ...
  override identifier(): string {
    return `eager (${this._shareScope}) ${this._name}@${this._version} = ${this._request}`;
  }
  ...
  override readableIdentifier(requestShortener: RequestShortener): string {
    return `eager module (${this._shareScope}) ${this._name}@${
      this._version
  ...
  override libIdent(options: LibIdentOptions): string | null {
    return `${this.layer ? `(${this.layer})/` : ''}webpack/sharing/eager/${
      this._shareScope
    }/${this._name}`;
  }
  ...
  override build(
    ...
    const dep = new ProvideForEagerDependency(this._request);
    if (this._eager) {
  ...
  override codeGeneration({
    ...
    const code = `hodor(${JSON.stringify(this._name)}, ${JSON.stringify(
      this._version || '0',
   ...
   * @returns {ProvideEagerModule} deserialize fallback dependency
   */
  static deserialize(context: ObjectDeserializerContext): ProvideEagerModule {
    const { read } = context;
    const obj = new ProvideEagerModule(
      read(),
      read(),
      read(),
      read(),
...
makeSerializable(ProvideEagerModule, 'enhanced/lib/sharing/ProvideEagerModule');

export default ProvideEagerModule;
