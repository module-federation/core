...
import type { Compiler, Compilation, ChunkGroup, Chunk, NormalModule } from 'webpack';
...
import { WEBPACK_LAYERS } from  '../../constants'
  ...
  createAsset(compilation: Compilation, context: string) {
    ...
    for (let [entryName, entrypoint] of compilation.entrypoints) {
      ...
      manifest.entryCSSFiles[chunkEntryName] = entrypoint
        .getFiles()
        .filter((f: any) => !f.startsWith('static/css/pages/') && f.endsWith('.css'))
        .map((file: any) => {
