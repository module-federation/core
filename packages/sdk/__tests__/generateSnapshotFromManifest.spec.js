...
describe('generateSnapshotFromManifest', function () {
  it('return basic app snapshot with only manifest params in dev', function () {
    var remoteSnapshot = (0,
    generateSnapshotFromManifest_1.generateSnapshotFromManifest)(
      manifestSnapshotMap_1.manifest.devAppManifest,
    );
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshot,
    );
  });
  it('return modulePath while expose.path existed', function () {
    ...
    manifestSnapshotMap_1.manifest.devAppManifest.exposes.forEach(
      function (expose) {
        if (expose.path) {
          expect(
            Boolean(
              remoteSnapshot.modules.find(function (m) {
                return m.modulePath === expose.path;
              }),
            ),
          ).toEqual(true);
        }
  ...
  it('return basic app snapshot with manifest params and version in dev', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithVersion,
    );
  });
  it('return basic app snapshot with only manifest params in dev with getPublicPath', function () {
    var remoteSnapshot = (0,
    generateSnapshotFromManifest_1.generateSnapshotFromManifest)(
      manifestSnapshotMap_1.manifest.devAppManifestWithGetPublicPath,
    );
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithGetPublicPath,
    );
  });
  it('return app snapshot with manifest params in dev with overrides', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithOverrides,
    );
  });
  it('return app snapshot with manifest params in dev with part remotes', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithPartRemotes,
    );
  });
  it('return app snapshot with manifest params in dev with remotes', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithRemotes,
    );
  });
  it('return app snapshot with manifest params in dev with remotes and overrides', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithRemotesAndOverrides,
    );
  });
  it('return app snapshot with manifest params in dev with all params', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.devAppSnapshotWithAllParams,
    );
  });
  it('return basic app snapshot with only manifest params in prod', function () {
    var remoteSnapshot = (0,
    generateSnapshotFromManifest_1.generateSnapshotFromManifest)(
      manifestSnapshotMap_1.manifest.prodAppManifest,
    );
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.prodAppSnapshot,
    );
  });
  it('return basic app snapshot with only manifest params in prod with getPublicPath', function () {
    var remoteSnapshot = (0,
    generateSnapshotFromManifest_1.generateSnapshotFromManifest)(
      manifestSnapshotMap_1.manifest.prodAppManifestWithGetPublicPath,
    );
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.prodAppSnapshotWithGetPublicPath,
    );
  });
  it('return app snapshot with manifest params in prod with all params', function () {
    ...
    expect(remoteSnapshot).toEqual(
      manifestSnapshotMap_1.snapshot.prodAppSnapshotWithAllParams,
    );
  });
});
