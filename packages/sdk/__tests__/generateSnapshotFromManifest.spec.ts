...
describe('generateSnapshotFromManifest', () => {
    it('return basic app snapshot with only manifest params in dev', () => {
        const remoteSnapshot = generateSnapshotFromManifest( manifest.devAppManifest);
    ...
    it('return modulePath while expose.path existed', () => {
        ...
        manifest.devAppManifest.exposes.forEach(expose=>{
            if(expose.path){
                expect(Boolean(remoteSnapshot.modules.find(m=>m.modulePath===expose.path))).toEqual(true)
            }
        })
    });

    it('return basic app snapshot with manifest params and version in dev', () => {
        const remoteSnapshot = generateSnapshotFromManifest( manifest.devAppManifest,{version:'http://localhost:2006/vmok-manifest.json'});
    ...
    it('return basic app snapshot with only manifest params in dev with getPublicPath', () => {
        const remoteSnapshot = generateSnapshotFromManifest(manifest.devAppManifestWithGetPublicPath);
    ...
    it('return app snapshot with manifest params in dev with remotes and overrides', () => {
        ...
        expect(remoteSnapshot).toEqual(snapshot.devAppSnapshotWithRemotesAndOverrides)
    });
    ...
    it('return basic app snapshot with only manifest params in prod', () => {
        const remoteSnapshot = generateSnapshotFromManifest(manifest.prodAppManifest);
    ...
    it('return basic app snapshot with only manifest params in prod with getPublicPath', () => {
        const remoteSnapshot = generateSnapshotFromManifest(manifest.prodAppManifestWithGetPublicPath);
