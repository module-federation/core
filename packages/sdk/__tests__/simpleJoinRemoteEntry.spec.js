...
describe('simpleJoinRemoteEntry', function () {
    it('return remoteEntryName while remoteEntryPath is empty', function () {
        ...
        expect((0, generateSnapshotFromManifest_1.simpleJoinRemoteEntry)(remoteEntry.path, remoteEntry.name)).toEqual(remoteEntry.name);
    });
    ...
    it('normalize remoteEntryPath while remoteEntryPath startsWith "./"', function () {
        ...
        expect((0, generateSnapshotFromManifest_1.simpleJoinRemoteEntry)(remoteEntry.path, remoteEntry.name)).toEqual("dist/vmok/".concat(remoteEntry.name));
    });
    it('normalize remoteEntryPath while remoteEntryPath startsWith "/"', function () {
        var remoteEntry = {
            name: 'vmok-remote-entry.js',
