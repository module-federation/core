...
describe('simpleJoinRemoteEntry', () => {
  it('return remoteEntryName while remoteEntryPath is empty', () => {
    ...
    expect(simpleJoinRemoteEntry(remoteEntry.path,remoteEntry.name)).toEqual(remoteEntry.name);
  });
  ...
  it('normalize remoteEntryPath while remoteEntryPath startsWith "./"', () => {
    ...
    expect(simpleJoinRemoteEntry(remoteEntry.path,remoteEntry.name)).toEqual(`dist/vmok/${remoteEntry.name}`);
  });
