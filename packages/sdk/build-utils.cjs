 ...
 */
async function fixTypesForNodeNext(options = {}) {
  ...
  try {
    ...
    function copyTypeFilesRecursively(srcPath, distPath, relativePath = '') {
      let copiedCount = 0;
    ...
    try {
      ...
      if (verbose) {
        console.error('❌ Error copying additional type files:', error.message);
      }
    }
 ...
 */
function createNodeNextTypeFixPlugin(options = {}) {
  return {
