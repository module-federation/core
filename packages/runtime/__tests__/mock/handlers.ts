...
const mainMainifestUrl = 'http://localhost:1111/resources/main/federation-manifest.json';

export const handlers = [
  rest.get(mainMainifestUrl, (req, res, ctx) => {
    requestList.set(mainMainifestUrl, (requestList.get(mainMainifestUrl) || 0) + 1);
  ...
  rest.get('http://localhost:1111/resources/:category/:app/:file', (req, res, ctx) => {
    const category = req.params.category;
    const app = req.params.app;
    const file = req.params.file;
    let filepath = path.resolve(__dirname, `../resources/${category}/${app}/${file}`)
