...
module.exports = (rollupConfig, projectOptions) => {
  ...
  const RELEASE_NUMBER = Number(`${semver.major(pkg.version)}${semver.minor(pkg.version)}${semver.patch(pkg.version)}`)

  if(Number.isNaN(RELEASE_NUMBER)){
    throw new Error(`RELEASE_NUMBER is not valid number , please check current pkg.version is valid.`)
  }
