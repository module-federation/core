...
declare class ChunkTemplate {
    ...
    hooks: Readonly<{
        ...
        modules: {
            tap: <AdditionalOptions>(options: string | (Tap & IfSet<AdditionalOptions>), fn: (source: Source, moduleTemplate: ModuleTemplate, renderContext: RenderContext) => Source) => void;
        };
        render: {
            tap: <AdditionalOptions>(options: string | (Tap & IfSet<AdditionalOptions>), fn: (source: Source, moduleTemplate: ModuleTemplate, renderContext: RenderContext) => Source) => void;
        };
        renderWithEntry: {
            tap: <AdditionalOptions>(options: string | (Tap & IfSet<AdditionalOptions>), fn: (source: Source, chunk: Chunk) => Source) => void;
        };
        hash: {
            tap: <AdditionalOptions>(options: string | (Tap & IfSet<AdditionalOptions>), fn: (hash: Hash) => void) => void;
        };
        hashForChunk: {
            tap: <AdditionalOptions>(options: string | (Tap & IfSet<AdditionalOptions>), fn: (hash: Hash, chunk: Chunk, chunkHashContext: ChunkHashContext) => void) => void;
        };
    }>;
    get outputOptions(): import("./config/defaults").OutputNormalizedWithDefaults;
}
declare namespace ChunkTemplate {
    export { Tap, OutputOptions, Chunk, Compilation, ChunkHashContext, Hash, RenderManifestEntry, RenderManifestOptions, Source, ModuleTemplate, RenderContext, IfSet };
}
