...
declare class CacheFacade {
     ...
     */
    constructor(cache: Cache, name: string, hashFunction?: (string | HashConstructor) | undefined);
    _cache: import("./Cache");
    _name: string;
     ...
     * @returns {Promise<T>} promise with the data
     */
    providePromise<T>(identifier: string, etag: Etag | null, computer: () => Promise<T> | T): Promise<T>;
}
declare namespace CacheFacade {
    export { ItemCacheFacade, MultiItemCache, Cache, Etag, HashableObject, HashConstructor, CallbackCache, CallbackNormalErrorCache };
}
...
type CallbackCache<T> = (err?: (Error | null) | undefined, result?: (T | null) | undefined) => void;
