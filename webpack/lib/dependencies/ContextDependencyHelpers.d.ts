export function create(Dep: ContextDependencyConstructor, range: Range, param: BasicEvaluatedExpression, expr: Expression, options: Pick<JavascriptParserOptions, `${"expr" | "wrapped"}Context${"Critical" | "Recursive" | "RegExp"}` | "exprContextRequest">, contextOptions: PartialContextDependencyOptions, parser: JavascriptParser, ...depArgs: EXPECTED_ANY[]): ContextDependency;
...
export type ContextDependencyConstructor = {
    new (options: ContextDependencyOptions, range: Range, valueRange: Range, ...args: EXPECTED_ANY[]): ContextDependency;
};
