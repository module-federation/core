...
export function createArrayToSetDeprecationSet<T>(name: string): {
  new <T_1 = EXPECTED_ANY>(
    values?: ReadonlyArray<T_1> | null,
  ): {
...
export function createFakeHook<T extends unknown>(
  fakeHook: T,
  message?: string | undefined,
  code?: string | undefined,
): FakeHook<T>;
export function soonFrozenObjectDeprecation<T extends unknown>(
  obj: T,
  name: string,
  code: string,
  note?: string,
...
export type COPY_METHODS_NAMES =
  | 'concat'
  | 'entry'
  | 'filter'
  | 'find'
  ...
  | 'slice'
  | 'some';
export type DISABLED_METHODS_NAMES =
  | 'copyWithin'
  | 'entries'
  | 'fill'
  | 'keys'
...
export type SetWithDeprecatedArrayMethods<T> = Set<T> & {
  ...
  push: (...items: T[]) => void;
  length?: number;
} & { [P in DISABLED_METHODS_NAMES]: () => void } & {
  [P in COPY_METHODS_NAMES]: P extends keyof Array<T>
    ? () => Pick<Array<T>, P>
    : never;
};
/**
