...
declare namespace DefaultStatsFactoryPlugin {
    export { Source, StatsValue, StatsFactory, StatsFactoryContext, Chunk, ChunkId, ChunkName, ModuleId, ChunkGroup, OriginRecord, Compilation, Asset, AssetInfo, ExcludeModulesType, KnownNormalizedStatsOptions, NormalizedStatsOptions, Compiler, Dependency, DependencyLocation, Module, NameForCondition, BuildInfo, ModuleGraphConnection, ModuleProfile, WebpackError, AggregateError, ErrorWithCause, ExportInfoName, Comparator, GroupConfig, StatsCompilation, KnownStatsCompilation, StatsLogging, KnownStatsLogging, StatsLoggingEntry, KnownStatsLoggingEntry, StatsAsset, ChunkIdHints, KnownStatsAsset, StatsChunkGroup, KnownStatsChunkGroup, ModuleIssuerPath, StatsModule, KnownStatsModule, StatsProfile, KnownStatsProfile, StatsModuleIssuer, KnownStatsModuleIssuer, StatsModuleReason, KnownStatsModuleReason, StatsChunk, KnownStatsChunk, StatsChunkOrigin, KnownStatsChunkOrigin, StatsModuleTraceItem, KnownStatsModuleTraceItem, StatsModuleTraceDependency, KnownStatsModuleTraceDependency, StatsError, KnownStatsError, PreprocessedAsset, ExtractorsByOption, ChunkGroupInfoWithName, ModuleTrace, SimpleExtractors, MappedValues, Children, ModuleGroupBySizeResult, BaseGroup, BaseGroupWithChildren, AssetsGroupers, ModulesGroupers, ModuleReasonsGroupers, AssetSorters, ExtractFunction, NamedObject };
}
...
type AggregateError = import("../serialization/AggregateErrorSerializer").AggregateError;
type ErrorWithCause = import("../serialization/ErrorObjectSerializer").ErrorWithCause;
...
type StatsModuleTraceItem = KnownStatsModuleTraceItem & Record<string, EXPECTED_ANY>;
...
type StatsModuleTraceDependency = KnownStatsModuleTraceDependency & Record<string, EXPECTED_ANY>;
...
type ExtractorsByOption<T_1, O> = Record<string, (object: O, data: T_1, context: StatsFactoryContext, options: NormalizedStatsOptions, factory: StatsFactory) => void>;
...
type AssetsGroupers = {
    _: (groupConfigs: GroupConfig<KnownStatsAsset, (BaseGroup & {
        filteredChildren: number;
        size: number;
    }) | BaseGroupWithChildren<KnownStatsAsset>>[], context: StatsFactoryContext, options: NormalizedStatsOptions) => void;
    ...
    excludeAssets: (groupConfigs: GroupConfig<KnownStatsAsset, BaseGroup & {
        filteredChildren: number;
        size: number;
    }>[], context: StatsFactoryContext, options: NormalizedStatsOptions) => void;
};
type ModulesGroupers = {
    _: (groupConfigs: GroupConfig<KnownStatsModule, BaseGroup & {
        filteredChildren?: number;
        children?: KnownStatsModule[];
        size: number;
        sizes: Record<string, number>;
    }>[], context: StatsFactoryContext, options: NormalizedStatsOptions) => void;
    excludeModules: (groupConfigs: GroupConfig<KnownStatsModule, BaseGroup & {
        filteredChildren: number;
        size: number;
        sizes: Record<string, number>;
    }>[], context: StatsFactoryContext, options: NormalizedStatsOptions) => void;
...
type ModuleReasonsGroupers = {
    groupReasonsByOrigin: (groupConfigs: GroupConfig<KnownStatsModuleReason, BaseGroup & {
        module: string;
...
type ExtractFunction<T_1> = T_1 extends Record<string, Record<string, infer F>> ? F : never;
