...
type NamedPresets = {
  [Key in Exclude<
    StatsValue,
    boolean | EXPECTED_OBJECT | 'normal'
  >]: StatsOptions;
};
type DefaultsKeys = keyof NormalizedStatsOptions;
type Defaults = {
  [Key in DefaultsKeys]: (
    options: Partial<NormalizedStatsOptions>,
    context: CreateStatsOptionsContext,
    compilation: Compilation,
  ) => NormalizedStatsOptions[Key] | RequestShortener;
};
...
type Normalizers = {
  [Key in NormalizerKeys]?: (
    value: StatsOptions[Key],
  ) => KnownNormalizedStatsOptions[Key];
};
