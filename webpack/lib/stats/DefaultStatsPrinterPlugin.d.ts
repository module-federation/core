...
declare namespace DefaultStatsPrinterPlugin {
  export {
    ...
    Exclamation,
    Printers,
    CompilationSimplePrinters,
    AssetSimplePrinters,
    ModuleSimplePrinters,
    ModuleIssuerPrinters,
    ModuleReasonsPrinters,
    ModuleProfilePrinters,
    ChunkGroupPrinters,
    ChunkPrinters,
    ErrorPrinters,
    LogEntryPrinters,
    ModuleTraceItemPrinters,
    ModuleTraceDependencyPrinters,
    SimpleItemsJoiner,
    Item,
    SimpleElementJoiner,
    Tail,
...
type KnownStatsChunkGroup =
  import('./DefaultStatsFactoryPlugin').KnownStatsChunkGroup;
type KnownStatsChunkOrigin =
  import('./DefaultStatsFactoryPlugin').KnownStatsChunkOrigin;
type KnownStatsCompilation =
  import('./DefaultStatsFactoryPlugin').KnownStatsCompilation;
type KnownStatsError = import('./DefaultStatsFactoryPlugin').KnownStatsError;
type KnownStatsLogging =
  import('./DefaultStatsFactoryPlugin').KnownStatsLogging;
type KnownStatsLoggingEntry =
  import('./DefaultStatsFactoryPlugin').KnownStatsLoggingEntry;
type KnownStatsModule = import('./DefaultStatsFactoryPlugin').KnownStatsModule;
type KnownStatsModuleIssuer =
  import('./DefaultStatsFactoryPlugin').KnownStatsModuleIssuer;
type KnownStatsModuleReason =
  import('./DefaultStatsFactoryPlugin').KnownStatsModuleReason;
type KnownStatsModuleTraceDependency =
  import('./DefaultStatsFactoryPlugin').KnownStatsModuleTraceDependency;
type KnownStatsModuleTraceItem =
  import('./DefaultStatsFactoryPlugin').KnownStatsModuleTraceItem;
type KnownStatsProfile =
  import('./DefaultStatsFactoryPlugin').KnownStatsProfile;
...
type KnownStatsPrinterColorFunctions =
  import('./StatsPrinter').KnownStatsPrinterColorFunctions;
type KnownStatsPrinterContext =
  import('./StatsPrinter').KnownStatsPrinterContext;
type KnownStatsPrinterFormatters =
  import('./StatsPrinter').KnownStatsPrinterFormatters;
type StatsPrinterContext = import('./StatsPrinter').StatsPrinterContext;
type StatsPrinterContextWithExtra =
  import('./StatsPrinter').StatsPrinterContextWithExtra;
type WithRequired<T, K extends keyof T> = { [P in K]-?: T[P] };
type DefineStatsPrinterContext<
  RequiredStatsPrinterContextKeys extends keyof StatsPrinterContext,
> = StatsPrinterContextWithExtra &
  WithRequired<
    StatsPrinterContext,
    'compilation' | RequiredStatsPrinterContextKeys
  >;
type SimplePrinter<
  T,
  RequiredStatsPrinterContextKeys extends keyof StatsPrinterContext,
> = (
  thing: Exclude<T, undefined>,
  context: DefineStatsPrinterContext<RequiredStatsPrinterContextKeys>,
...
type PropertyName<
  ...
  K extends keyof O,
  B extends string,
> = K extends string ? `${B}.${K}` : never;
type ArrayPropertyName<
  O extends unknown,
  K extends keyof O,
  B extends string,
> = K extends string ? `${B}.${K}[]` : never;
type Exclamation<O extends unknown, K extends string, E extends string> = {
  [property in `${K}!`]?: SimplePrinter<O, 'compilation' | E>;
};
type Printers<O extends unknown, B extends string, R extends string = B> = {
  [K in keyof O as PropertyName<O, K, B>]?: SimplePrinter<O[K], R>;
} & {
  [K in keyof O as ArrayPropertyName<O, K, B>]?: Exclude<
    O[K],
    undefined
  > extends (infer I)[]
    ? SimplePrinter<I, R>
...
type CompilationSimplePrinters = Printers<
  KnownStatsCompilation,
  'compilation'
> & {
  ...
  ['compilation.warningsInChildren!']?: SimplePrinter<
    KnownStatsCompilation,
    'compilation'
  >;
};
type AssetSimplePrinters = Printers<KnownStatsAsset, 'asset'> &
...
type ModuleSimplePrinters = Printers<KnownStatsModule, 'module'> &
...
type ModuleIssuerPrinters = Printers<KnownStatsModuleIssuer, 'moduleIssuer'> &
  Printers<
    KnownStatsModuleIssuer['profile'],
    'moduleIssuer.profile',
    'moduleIssuer'
  >;
type ModuleReasonsPrinters = Printers<
  KnownStatsModuleReason,
  'moduleReason'
> & {
  ['moduleReason.filteredChildren']?: SimplePrinter<number, 'moduleReason'>;
};
type ModuleProfilePrinters = Printers<
  KnownStatsProfile,
  'module.profile',
  'profile'
>;
type ChunkGroupPrinters = Exclamation<
  KnownStatsChunkGroup,
  'chunkGroup.kind',
  'chunkGroupKind'
> &
  ...
  Printers<
    Exclude<KnownStatsChunkGroup['assets'], undefined>[number],
    'chunkGroupAsset' | 'chunkGroup'
  > & {
...
type ChunkPrinters = Printers<KnownStatsChunk, 'chunk'> & {
  ...
  Printers<KnownStatsChunkOrigin, 'chunkOrigin'>;
...
type LogEntryPrinters = Printers<
  KnownStatsLoggingEntry,
  `loggingEntry(${LogTypeEnum}).loggingEntry`
> & {
  ...
  ['loggingEntry.trace[]']?: SimplePrinter<
    Exclude<KnownStatsLoggingEntry['trace'], undefined>[number],
    'logging'
  >;
} & {
  loggingGroup?: SimplePrinter<KnownStatsLogging[], 'logging'>;
    ...
    `loggingGroup`
  > &
  Exclamation<KnownStatsLogging, 'loggingGroup.separator', 'loggingGroup'>;
type ModuleTraceItemPrinters = Printers<
  KnownStatsModuleTraceItem,
  'moduleTraceItem'
>;
type ModuleTraceDependencyPrinters = Printers<
  KnownStatsModuleTraceDependency,
...
type SimpleElementJoiner = (
  items: Item[],
  context: StatsPrinterContextWithExtra & Required<KnownStatsPrinterContext>,
) => string;
type Tail<T> = T extends [infer Head, ...infer Tail] ? Tail : undefined;
type TailParameters<T extends (...args: EXPECTED_ANY[]) => EXPECTED_ANY> =
  T extends (firstArg: EXPECTED_ANY, ...rest: infer R) => EXPECTED_ANY
    ? R
    : never;
type AvailableFormats = {
  [Key in keyof KnownStatsPrinterFormatters]: (
    value: Parameters<NonNullable<KnownStatsPrinterFormatters[Key]>>[0],
    options: Required<KnownStatsPrinterColorFunctions> &
      StatsPrinterContextWithExtra,
    ...args: TailParameters<NonNullable<KnownStatsPrinterFormatters[Key]>>
  ) => string;
};
