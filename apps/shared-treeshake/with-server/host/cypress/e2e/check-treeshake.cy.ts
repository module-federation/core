describe('/', () => {
  ...
  describe('Check shared treeshake', () => {
    it('should load treeshake shared if set strategy "server" with snapshot', () => {
      cy.window().should((win) => {
        const shared =
          win.__FEDERATION__?.__SHARE__['mf_host:0.1.34'].default.antd['6.0.1'];
        expect(shared).to.exist;
        expect(shared.treeshake.strategy).to.equal('server');
        expect(shared.treeshake.loaded).to.equal(true);
        expect(Object.keys(shared.treeshake.lib()).sort()).to.deep.equal([
          'Badge',
          'Button',
          'Divider',
          'Space',
