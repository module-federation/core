...
describe('HMR Client Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockCustomHMRHelpers = {
      applyHotUpdateFromStringsByPatching: async (
        moduleObj,
        webpackRequire,
        manifestJsonString,
        chunkJsStringsMap,
        ...
        if (moduleObj.hot.status() !== 'idle') {
          throw new Error(
            `[HMR] Cannot apply update, status: ${moduleObj.hot.status()}`,
          );
        }
    ...
    require.cache[
      require.resolve('@module-federation/node/utils/custom-hmr-helpers')
    ].exports = mockCustomHMRHelpers;
  ...
  afterEach(() => {
    ...
    require.cache[
      require.resolve('@module-federation/node/utils/custom-hmr-helpers')
    ].exports = originalCustomHMRHelpers;
  });
  ...
  describe('Update Operations', () => {
    ...
    it('should apply update successfully', async () => {
      ...
      assert.strictEqual(
        stats.totalUpdates,
        1,
        'Should track the update attempt',
      );
      assert.ok(
    ...
    it('should force update with minimal update creation', async () => {
      ...
      assert.strictEqual(
        stats.totalUpdates,
        1,
        'Should track the force update attempt',
      );
    });
  ...
  describe('Polling Operations', () => {
    ...
    it('should handle polling callbacks', async () => {
      ...
      assert.ok(
        errorCallbackCalled,
        'Should call error callback for no updates',
      );
    });
  ...
  describe('Status and Statistics', () => {
    it('should get correct status', () => {
      ...
      assert.ok(
        typeof status.hasModuleHot === 'boolean',
        'hasModuleHot should be a boolean',
      );
      assert.ok(
        typeof status.hotStatus === 'string',
        'hotStatus should be a string',
      );
      assert.ok(
        typeof status.webpackHash === 'string' || status.webpackHash === null,
        'webpackHash should be string or null',
      );
    ...
    it('should track statistics correctly', async () => {
      ...
      assert.ok(
        stats.lastUpdateTime !== null || stats.failedUpdates > 0,
        'Should have update activity',
      );
    });
  ...
  describe('Integration Scenarios', () => {
    it('should handle complete update workflow', async () => {
      ...
      assert.strictEqual(
        stats.totalUpdates,
        stats.successfulUpdates + stats.failedUpdates,
        'Total should equal sum of success and failures',
      );
    });
