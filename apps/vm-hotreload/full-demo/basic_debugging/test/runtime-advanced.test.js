...
describe('Advanced HMR Runtime Tests', () => {
  ...
  describe('HMR Handlers Deep Testing', () => {
    it('should test hmrI handler with complex module scenarios', () => {
      ...
      assert.ok(Array.isArray(state.currentUpdateRemovedChunks), 'Should initialize removed chunks array');
    ...
    it('should test hmrC handler with multiple chunk configurations', async () => {
      ...
      assert.ok(state.currentUpdate['./src/removed-module.js'] === false, 'Removed modules should be marked false');
      assert.ok(Array.isArray(state.currentUpdateRuntime), 'Should initialize runtime array');
      
      // Check chunk processing logic
      assert.ok(state.currentUpdateChunks['index'] === true, 'Installed chunk should be marked for loading');
      assert.ok(state.currentUpdateChunks['vendor'] === true, 'Installed chunk should be marked for loading');
      assert.ok(state.currentUpdateChunks['main'] === false, 'Uninstalled chunk should be skipped');
    });
  ...
  describe('Load Update Chunk Edge Cases', () => {
    it('should handle in-memory chunks with complex module exports', async () => {
      ...
      assert.ok(state.currentUpdate['./src/complex-a.js'], 'Should load complex-a module');
      assert.ok(state.currentUpdate['./src/complex-b.js'], 'Should load complex-b module');
      assert.ok(state.currentUpdateRuntime.length > 0, 'Should load runtime');
      assert.ok(updatedModulesList.includes('./src/complex-a.js'), 'Should track updated modules');
      assert.ok(updatedModulesList.includes('./src/complex-b.js'), 'Should track updated modules');
    });
    ...
    it('should handle chunks with circular dependencies', async () => {
      ...
      assert.ok(state.currentUpdate['./src/circle-a.js'], 'Should load circle-a module');
      assert.ok(state.currentUpdate['./src/circle-b.js'], 'Should load circle-b module');
    });
    ...
    it('should handle filesystem fallback when chunks not in memory', async () => {
      ...
      try {
        ...
        assert.ok(state.currentUpdate['./src/fallback-module.js'], 'Should load module from filesystem');
      } finally {
  ...
  describe('HMR Manifest Loader Advanced Tests', () => {
    it('should handle complex manifest structures', async () => {
      ...
      const manifestLoader = createHMRManifestLoader(mockWebpackRequire, manifestRef);

      const loadedManifest = await manifestLoader();

      assert.deepStrictEqual(loadedManifest, complexManifest, 'Should load complex manifest correctly');
      assert.ok(loadedManifest.metadata, 'Should preserve metadata');
      assert.strictEqual(loadedManifest.metadata.version, '2.0.0', 'Should preserve version info');
    });
    ...
    it('should handle filesystem manifest loading with complex paths', async () => {
      ...
      try {
        ...
        assert.strictEqual(manifest.h, 'filesystem-hash', 'Should load correct hash');
        assert.ok(manifest.c.includes('filesystem-chunk'), 'Should load chunk info');
      } finally {
        fs.readFile = originalReadFile;
      }
    });
    ...
    it('should handle missing manifest files gracefully', async () => {
      ...
      try {
        ...
        assert.strictEqual(manifest, undefined, 'Should return undefined for missing manifest');
      } finally {
        fs.readFile = originalReadFile;
      }
    });
  ...
  describe('Apply Handler Complex Scenarios', () => {
    it('should handle modules with complex hot acceptance patterns', () => {
      ...
      mockWebpackRequire.c = {
        './src/acceptor.js': {
          ...
          hot: {
            ...
            _disposeHandlers: [
              function(data) { data.disposed = true; },
            ],
            active: true,
      ...
      const installedChunks = { index: 0 };
      const state = {
        currentUpdate: { './src/dep-a.js': function() { return 'updated-dep-a'; } },
        currentUpdateRuntime: [],
      ...
      if (result.apply) {
        ...
        assert.ok(Array.isArray(outdatedModules), 'Should return outdated modules array');
      }
    });
    ...
    it('should handle runtime module execution with side effects', () => {
      ...
      if (result.apply) {
        ...
        assert.deepStrictEqual(runtimeExecuted, ['runtime-1', 'runtime-2'], 'Should execute runtime modules in order');
        assert.strictEqual(mockWebpackRequire.customProperty, 'set-by-runtime-1', 'Should apply runtime side effects');
        assert.strictEqual(mockWebpackRequire.anotherProperty, 'set-by-runtime-2', 'Should apply runtime side effects');
      }
    });
  });
  ...
  describe('Complete Runtime Integration', () => {
    it('should handle full runtime lifecycle with all components', async () => {
      ...
      assert.strictEqual(manifest.h, 'integration-hash', 'Should load correct manifest');
      ...
      if (applyResult.apply) {
        applyResult.apply(() => {});
        assert.ok(mockWebpackRequire.mainRuntimeExecuted, 'Should execute runtime modules');
      }
    });
  });
});