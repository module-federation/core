...
describe('End-to-End Webpack HMR Tests', () => {
  ...
  describe('HMR Client Demo End-to-End Tests', () => {
    ...
    it('should build hmr-client-demo entrypoint successfully', () => {
      assert.ok(buildSuccessful, 'Build should have completed successfully');
      ...
      // Verify HMR Client library is included
      assert.ok(
      ...
      // Verify webpack runtime
      assert.ok(
        builtContent.includes('__webpack_require__'),
        'Should contain webpack runtime',
      );
    });

    it('should run HMR Client Demo successfully', (t, done) => {
      if (!buildSuccessful) {
        done(new Error('Build was not successful'));
        return;
      }
      ...
      child.stdout.on('data', (data) => {
        ...
        if (stdout.includes('HMR Client Demo completed successfully')) {
          demoCompleted = true;
      ...
      child.on('close', (code) => {
        try {
          ...
          assert.ok(demoCompleted, 'Demo should have completed');
          ...
          assert.ok(
            stdout.includes('Demo functionality tests'),
            'Should run demo tests',
          );
          assert.ok(
            stdout.includes('automatic update demonstration'),
            'Should demonstrate updates',
          );
    ...
    it('should demonstrate HMR Client library features', (t, done) => {
      if (!buildSuccessful) {
        done(new Error('Build was not successful'));
        return;
      }
      ...
      // Create a test script that uses the built HMR Client Demo application
      const testScript = `
        const hmrDemo = require('${hmrClientDemoPath}');

        console.log('Testing HMR Client Demo library features...');

        // Test the HMR Client instance
        const client = hmrDemo.hmrClient;
        console.log('HMR Client status:', client.getStatus().isAttached ? 'ATTACHED' : 'NOT_ATTACHED');
        console.log('HMR Client stats:', JSON.stringify(client.getStats()));

        // Test demo API
        console.log('Greeting test:', hmrDemo.demoAPI.greet('Tester'));

        // Test calculation
        const calcResult = hmrDemo.demoAPI.calculate(10);
        console.log('Calculation result:', JSON.stringify(calcResult));

        // Test data processing
        const testData = [{ id: 1, name: 'Test' }];
        const processed = hmrDemo.demoAPI.processData(testData);
        console.log('Processed data:', JSON.stringify(processed));

        // Test status retrieval
        const status = hmrDemo.demoAPI.getStatus();
        console.log('Demo status version:', status.version);

        // Test update provider functionality
        const updateProvider = client.updateProvider;
        console.log('Update provider configured:', updateProvider ? 'YES' : 'NO');

        console.log('All HMR Client Demo library tests passed!');
        process.exit(0);
      `;

      const testScriptPath = path.join(distDir, 'test-hmr-client-demo.js');
      fs.writeFileSync(testScriptPath, testScript);
      ...
      child.on('close', (code) => {
        try {
          ...
          assert.strictEqual(
            code,
            0,
            `HMR Client Demo library test should exit with code 0, got ${code}`,
          );
          assert.ok(
            stdout.includes('Testing HMR Client Demo library features'),
            'Should start library features test',
          );
          assert.ok(
            stdout.includes('HMR Client status: ATTACHED'),
            'Should show client as attached',
          );
          assert.ok(
            stdout.includes('Greeting test:'),
            'Should test greeting functionality',
          );
          assert.ok(
            stdout.includes('Calculation result:'),
            'Should test calculation functionality',
          );
          assert.ok(
            stdout.includes('All HMR Client Demo library tests passed'),
            'Should pass all tests',
          );

          if (stderr && !stderr.includes('DeprecationWarning')) {
            console.warn('HMR Client Demo library test stderr:', stderr);
          }

          console.log('✅ HMR Client Demo library integration verified');
          safeComplete();
        } catch (error) {
          console.log('HMR Client Demo library test stdout:', stdout);
          console.log('HMR Client Demo library test stderr:', stderr);
          safeComplete(error);
        }
      });
      ...
      // Timeout for library tests
      setTimeout(() => {
        if (!child.killed) {
          child.kill('SIGKILL');
          safeComplete(new Error('HMR Client Demo library test timed out'));
        }
      }, 15000);
    });

    it('should handle HMR Client updates in bundle environment', (t, done) => {
      if (!buildSuccessful) {
        done(new Error('Build was not successful'));
        return;
      }
      ...
      // Run the demo and verify it shows HMR Client update functionality
      const child = spawn('node', [hmrClientDemoPath], {
        cwd: projectRoot,
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      ...
      // Kill after sufficient time to see updates
      setTimeout(() => {
        if (!child.killed) {
          child.kill('SIGTERM');
        }
      ...
      child.on('close', (code) => {
        try {
          // Verify HMR Client update functionality in the output
          assert.ok(
            stdout.includes('Checking for updates'),
            'Should check for updates',
          );
          assert.ok(
            stdout.includes('Applying update') || stdout.includes('Update applied'),
            'Should apply updates',
          );
          assert.ok(
            stdout.includes('Final HMR Client Statistics'),
            'Should show final statistics',
          );
          assert.ok(
            stdout.includes('totalUpdates') || stdout.includes('successfulUpdates'),
            'Should show update statistics',
          );
          assert.ok(
            stdout.includes('HMR Client Demo completed successfully'),
            'Should complete demo successfully',
          );

          if (stderr && !stderr.includes('DeprecationWarning') && !stderr.includes('ENOENT')) {
            console.warn('HMR Client update test stderr:', stderr);
          }

          console.log('✅ HMR Client update functionality verified');
          safeComplete();
        } catch (error) {
          console.log('HMR Client update test stdout:', stdout);
          console.log('HMR Client update test stderr:', stderr);
          safeComplete(error);
        }
      });
      ...
      // Timeout for update tests
      setTimeout(() => {
        if (!child.killed) {
          child.kill('SIGKILL');
          safeComplete(new Error('HMR Client update test timed out'));
        }
      }, 15000);
    });
