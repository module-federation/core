...
describe('SSR + HMR Integration', () => {
  it('should initialize HMR-enabled modules with proper webpack structure', () => {
    ...
    assert.strictEqual(
      typeof leafModule.hot.accept,
      'function',
      'Module should have accept function',
    );
    assert.strictEqual(
      typeof leafModule.hot.dispose,
      'function',
      'Module should have dispose function',
    );
    assert.ok(
      moduleRegistry['test_leaf'],
      'Module should be registered in module registry',
    );
  });
  ...
  it('should render SSR HTML with module tracking attributes', () => {
    ...
    assert.ok(
      html.includes('data-module-id="leaf1"'),
      'Should include module ID for leaf1',
    );
    assert.ok(
      html.includes('data-module-id="leaf2"'),
      'Should include module ID for leaf2',
    );
  ...
  it('should simulate HMR updates and trigger hot reload handlers', () => {
    ...
    const leafModule = createLeafComponentModule(
      'Original Text',
      'hmr_test_leaf',
    );
    ...
    assert.strictEqual(
      leafModule.exports.getCurrentText(),
      'Updated Text',
      'Module text should be updated',
    );
    assert.ok(
      hotReloadTriggered,
      'Hot reload handler should have been triggered',
    );
  });
  ...
  it('should update SSR HTML after HMR simulation', () => {
    ...
    assert.ok(
      initialHTML.includes('Original 2'),
      'Initial HTML should have original text',
    );
    assert.ok(
      updatedHTML.includes('HMR Updated 2'),
      'Updated HTML should have HMR updated text',
    );
    assert.ok(
      !updatedHTML.includes('Original 2'),
      'Updated HTML should not have original text',
    );
    assert.ok(
      updatedHTML.includes('Original 1'),
  ...
  it('should maintain webpack module system integrity during HMR', () => {
    ...
    assert.ok(
      global.__webpack_require__.c[leaf.id],
      'Module should remain in module cache',
    );
  ...
  it('should run the complete SSR + HMR integration test successfully', async () => {
    ...
    assert.strictEqual(
      typeof result.hmrStats.totalUpdates,
      'number',
  ...
  it('should properly handle parent-child module relationships', () => {
    ...
    assert.ok(
      parentModule.children.includes('child'),
      'Parent should maintain child relationship',
    );
    assert.ok(
      childModule.parents.includes('parent'),
