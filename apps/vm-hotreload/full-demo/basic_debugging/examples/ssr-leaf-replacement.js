...
function testSSRLeafReplacement() {
  ...
  const initialLeaf2 = initialHTML.match(
    /<span[^>]*data-testid="leaf-leaf2"[^>]*>([^<]*)<\/span>/,
  );
  const updatedLeaf2 = updatedHTML.match(
    /<span[^>]*data-testid="leaf-leaf2"[^>]*>([^<]*)<\/span>/,
  );

  console.log('\nDetailed Leaf Node Analysis:');
  console.log(
    `Original leaf2 content: "${initialLeaf2 ? initialLeaf2[1] : 'NOT FOUND'}"`,
  );
  console.log(
    `Updated leaf2 content: "${updatedLeaf2 ? updatedLeaf2[1] : 'NOT FOUND'}"`,
  );
