...
function createFederationUpdateProvider() {
  ...
  return createCallbackUpdateProvider(async (currentHash) => {
    ...
    if (Math.random() < scenario.probability || updateCount <= 4) {
      const { content, hash, version } = await simulateRemoteEntryChange(
        scenario.page,
        scenario.version,
      );
      ...
      if (currentPageHash === hash) {
        console.log(
          `   No changes detected for ${scenario.page} (hash: ${hash})`,
        );
        return { update: null };
      }
...
async function runMultiEntryHMRTest() {
  ...
  console.log(
    '  - Each page has its own remote entry and federation container',
  );
  ...
  console.log(
    '--- Simulating the nuclear reset scenario from current issue ---',
  );
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(
        '   Module reloaded:',
        afterState.reloadCount > beforeState.reloadCount,
      );
      console.log(
        '   State reset:',
        !beforeState.isUpdated || afterState.isUpdated,
      );
    } else {
  ...
  console.log(
    '--- Testing concurrent updates to multiple federation containers ---',
  );

  const simultaneousUpdates = ['home', 'shop', 'about'].map(
    async (pageName, index) => {
      try {
        console.log(`🔄 Simulating update for ${pageName}...`);

        const { content, hash } = await simulateRemoteEntryChange(
          pageName,
          `2.${index}.0`,
        );
...
exports.modules = {
  ...
  results.forEach((result, index) => {
    if (result.status === 'fulfilled') {
      ...
      console.log(
        `   ${success ? '✅' : '❌'} ${pageName}: ${success ? 'Success' : 'Failed'}`,
      );
    } else {
