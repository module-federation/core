...
describe('Immediate HMR Test', () => {
  it('should immediately reset server render counter on same request with query string', () => {
    ...
    cy.get('div').contains(/^\d+$/).should(($el) => {
    ...
    cy.get('div').contains(/^\d+$/).should(($el) => {
      ...
      if (count === 1) {
        cy.log('✅ SUCCESS: Counter reset IMMEDIATELY without subsequent page load!')
      } else {
        cy.log('⚠️ Counter did not reset immediately - still needs subsequent page load')
      }
    })
  })
})