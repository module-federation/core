...
describe('Server Render Counter & HMR Integration', () => {
  ...
  const getCounterValue = () => {
    return cy.get('[data-testid="render-counter"]').should('exist').invoke('text').then(text => parseInt(text.trim()));
  };
  ...
  it('should validate HMR cache info API', () => {
    ...
    cy.request({
      ...
      expect(response.body.result.nativeAPIsAvailable.clearModuleContext).to.be.true;
      
      cy.log(`Cache info: ${response.body.result.totalCacheSize} modules, APIs available: ${JSON.stringify(response.body.result.nativeAPIsAvailable)}`);
    });
  });
  ...
  it('should show timestamp of last server render', () => {
    ...
    cy.get('div').contains('Last render:').should(($el) => {
      const timestampText = $el.text();
      const timeMatch = timestampText.match(/(\d{1,2}:\d{2}:\d{2})/);
      expect(timeMatch).to.not.be.null;
      cy.log(`Last server render timestamp: ${timeMatch ? timeMatch[1] : 'not found'}`);
    });
  });
  ...
  it('should test multiple HMR methods in sequence', () => {
    ...
    const methods = [
      { name: 'API clear-all-pages', action: () => {
        cy.request({
          method: 'POST',
          url: 'http://localhost:3000/api/server-hmr',
          headers: { 'Content-Type': 'application/json' },
      ...
      { name: 'Query ?clearCache=true', action: () => {
        cy.visit('http://localhost:3000?clearCache=true');
      }},
      { name: 'Query ?resetCounter=true', action: () => {
        cy.visit('http://localhost:3000?resetCounter=true');
      }}
    ];
