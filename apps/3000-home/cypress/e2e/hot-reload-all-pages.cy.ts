...
describe('Hot Reload All Pages', () => {
  ...
  it('should reset server render counter after HMR', () => {
    ...
    cy.contains(/Server Render Counter/i).parent().within(() => {
      cy.get('div').contains(/\d+/).then(($el) => {
        ...
        cy.contains(/Server Render Counter/i).parent().within(() => {
          cy.get('div').contains(/\d+/).then(($newEl) => {
  ...
  it('should preserve page functionality after hot reload', () => {
    ...
    cy.get('a[href*="/shop"], button, .ant-menu-item').first().then(($el) => {
      if ($el.length > 0) {
  ...
  it('should handle hot reload without breaking subsequent page loads', () => {
    ...
    navigationTests.forEach((url, index) => {
      ...
      cy.wait(1000)
      cy.get('body').should('be.visible')
      cy.log(`Navigation test ${index + 1}: ${url} loads successfully after hot reload`)
    })
  })
