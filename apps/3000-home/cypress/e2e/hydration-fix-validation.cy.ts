...
describe('Hydration Fix Validation E2E', () => {
  ...
  it('should prevent hydration errors when federation page is modified and reloaded', () => {
    ...
    cy.window().then((win) => {
      ...
      win.addEventListener('error', (e) => {
        if (
          e.message &&
          (e.message.includes('hydration') || e.message.includes('Hydration'))
        ) {
  ...
  it('should handle rapid modifications and multiple reloads', () => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      cy.get('body').should('contain', `${baseTestContent} ${i}`, {
        timeout: 10000,
      });
    ...
    pages.forEach((page, index) => {
      ...
      if (page === '/shop') {
        ...
        cy.get('body', { timeout: 8000 }).then(($body) => {
          ...
          const hasOldContent = text.includes(
            'This is a federated page owned by localhost:3001 (HYDRATION FIX TEST 123)',
          );
          const hasNewContent = text.includes(concurrentTestContent);

          if (!hasOldContent && !hasNewContent) {
            throw new Error(
              'Shop page content is neither old nor new - possible hydration corruption',
            );
          }
