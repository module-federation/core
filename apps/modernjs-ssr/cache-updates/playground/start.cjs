...
const dir = __dirname;
const installed =
  process.env.SSR_CACHE_PACKAGES_ROOT || path.resolve(dir, '..');
...
(async () => {
  const root =
    ...
    (await fs.mkdtemp(
      path.join(require('node:os').tmpdir(), 'ssr-playground-'),
    ));
  ...
  const selectedPackages = await require('./packages.cjs')(installed, root);
  const env = {
    ...process.env,
  ...
  server.listen(Number(process.env.SSR_CACHE_DEMO_PORT || 3058), '127.0.0.1');
