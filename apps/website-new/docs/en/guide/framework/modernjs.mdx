...
And then add `paths` to `tsconfig.json` to get type support:
...
- Type: `moduleFederationPlugin.ModuleFederationPluginOptions`
- Rquired: No
- Default Value: `undefined`

`@module-federation/modern-js` will auto read `module-federation.config.ts` config file. If `config` is configured, this configuration file will be used first.

### configPath

- Type: `string`
- Required: No
- Default Value: `path.resolve(process.cwd(), 'module-federation.config.ts');`

Specify the configuration file path, which must be an absolute path.
...
Setting `true` will automatically export the current project route.
...
After setting `true`, follow [this document](../../practice/frameworks/modern/ssr/route-component#how-to-enable-this-ability) to consume the routes exported by the producer.
...
When you need to obtain the Data Loader data of the specified route level, you need to wrap the original `routeId` with this function.
