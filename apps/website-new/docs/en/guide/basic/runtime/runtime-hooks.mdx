...
Called if loading remotes fails, enabling custom error handling. Can return a custom fallback logic. This hook is triggered at different lifecycle stages of remote loading.
    ...
    return {
      ...
      errorLoadRemote(args) {
        ...
        if (lifecycle === 'afterResolve') {
          ...
          console.warn(`Failed to load manifest for ${id}:`, error);
          return {
            id: 'fallback',
            name: 'fallback',
        ...
        // For other lifecycles, return original args to continue
        return args;
      },
    };
  };
