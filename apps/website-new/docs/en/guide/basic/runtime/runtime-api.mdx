...
:::note Node.js Federation Requirement
When using `createInstance` in a Node.js federated environment, you **must** also call `setGlobalFederationInstance` to register the instance globally. This is required for Node.js federation to work properly.

```ts
import { createInstance, setGlobalFederationInstance } from '@module-federation/enhanced/runtime';
...
// Required for Node.js federation
