...
```typescript title="List.tsx"
import React from 'react';
import type { Data } from './List.data';
...
```typescript title="List.data.client.ts"
import cache from 'my-cache';

export async function loader({ params }: { params: Record<string, string> }): Promise<{ message: string }> {
