...
Module Federation support for React Native applications using Metro bundler brings the power of distributed architecture to mobile development. This integration allows you to split your React Native applications into smaller, independently deployable modules that can be shared across different applications.
...
> **Note**: Module Federation support for Metro bundler is still experimental and may lack some functionality or certain integrations. Projects or libraries that rely on custom Metro configurations such as `Expo` or `NativeWind` aren't supported yet and might not work.
...
```shell
# Using npm
npm install @module-federation/metro @module-federation/metro-plugin-rnc-cli

# Using yarn
yarn add @module-federation/metro @module-federation/metro-plugin-rnc-cli
...
Wrap your Metro configuration with the `withModuleFederation` function to enable Module Federation:
...
### Bundle Remote Application

Bundle a mini application into static files:
...
Bundle a host application:

```shell
react-native bundle-mf-host --platform android --dev false --minify true
```
