...
In Module Federation, the host and remote applications run on the same page and share a single global CSS scope, so styles from one side can easily leak into another and cause unintended "style pollution". Module Federation does not ship built-in style isolation (see the [FAQ](#why-doesnt-module-federation-directly-handle-css-style-isolation) below for the reasoning), so producers should pick the strategy that fits their scenario.
...
CSS Modules achieve style isolation by treating each CSS file as a module, where each class name is transformed into a unique identifier during compilation. [Using CSS Modules in Rsbuild](https://v2.rsbuild.rs/guide/styling/css-modules)
...
CSS-in-JS writes styles directly in JavaScript files. Common libraries include [styled-components](https://styled-components.com/) and [emotion](https://github.com/emotion-js/emotion). [Using CSS-in-JS in Rsbuild](https://rsbuild.rs/guide/styling/css-in-js)
