...
## Installation and Runtime Import

Default projects should install `@module-federation/enhanced` and import Runtime APIs from `@module-federation/enhanced/runtime`. If your Modern.js project already uses `@module-federation/modern-js-v3`, import Runtime APIs from `@module-federation/modern-js-v3/runtime` so the framework plugin and manual Runtime calls use the same Runtime instance.
...
- [Runtime API](./runtime-api): `createInstance`, `loadRemote`, `registerRemotes`, and other runtime APIs.
- [Runtime Plugins](./runtime-plugins): extend the runtime loading flow.
- [Runtime Hooks](./runtime-hooks): lifecycle hooks available to runtime plugins.
