...
Called after the runtime attempts to register one version of a shared dependency
in a share scope. This hook reports both the candidate and the record that
actually remains registered, so it can also observe conflicts where an existing
provider wins.

```ts
function afterRegisterShare(args: AfterRegisterShareOptions): void;

type AfterRegisterShareOptions = {
  pkgName: string;
  scope: string;
  shared: Shared;
  ...
  trigger: 'build' | 'runtime' | 'container-init';
  origin: ModuleFederation;
};
```

- `shared` is the candidate the runtime tried to register.
- `previousShared` is the record at the same scope, package, and version before
  this attempt.
- `registeredShared` is the record that remains after conflict resolution.
- `trigger` identifies whether registration came from build-generated runtime
  code, a runtime API call, or remote container initialization.
...
`loadContext` carries the consumer that triggered shared resolution when that
information is available:

```ts
type SharedLoadContext = {
  trigger?: 'build' | 'runtime' | 'container-init';
...
Because `beforeLoadShare` is a waterfall hook, a plugin can return an updated
`loadContext`; the same value is then forwarded to `resolveShare`,
`afterLoadShare`, and `errorLoadShare`.
...
Called immediately before the runtime requests and parses a remote manifest.
It is not called when the manifest is already in the runtime cache.

```ts
async function beforeLoadManifest(
  args: BeforeLoadManifestOptions,
): Promise<void>;

type BeforeLoadManifestOptions = {
  manifestUrl: string;
  moduleInfo: Remote;
  resourceOptions?: {
...
Called after manifest loading finishes. It runs for network success, cache
hits, validation failures, and values recovered through `errorLoadRemote`.

```ts
async function afterLoadManifest(
  args: AfterLoadManifestOptions,
): Promise<void>;

type AfterLoadManifestOptions = {
  manifestUrl: string;
  moduleInfo: Remote;
  resourceOptions?: {
    initiator: 'loadRemote' | 'preloadRemote' | 'loadShare';
...
- `cached` is `true` when no network request was needed.
- `error` preserves the original loading or validation error.
- `recovered` is `true` when `errorLoadRemote` supplied a usable manifest.
...
`resourceContext` tells whether this resource load came from `loadRemote`,
`preloadRemote`, or shared initialization, plus the resource type, id, and
exposed module when applicable.
...
The `fetch` function allows customizing the requests used to load a remote. It is always applied to the manifest JSON request (a successful `Response` must yield a valid JSON). If a `fetch` hook is registered, it is additionally applied to the remote entry and manifest-declared CSS of **`module` / `esm`** remotes, which are then loaded through a `fetch` + blob loader so custom `headers` (e.g. `Authorization`) can be applied those requests too.
...
`resourceContext` can tell whether this manifest request came from
`loadRemote`, `preloadRemote`, or shared initialization.
...
`resourceContext` identifies the request that needed the remoteEntry. `cached`
is `true` when the exports were already available, while `recovered` means
`loadEntryError` returned replacement exports after a loading error.
...
`bridgeHook` observes React and Vue Bridge render, destroy, and route
synchronization operations. Bridge packages pass a semantic operation context
as a second argument while keeping the first `info` argument compatible with
existing plugins.
...
Triggered before a Bridge producer or consumer renders or updates. You can
return an object to augment render arguments (for example, `extraProps`).
...
function afterBridgeRender(
  info: Record<string, any>,
  operation?: BridgeOperationResult,
): void | Record<string, any>;
```
...
Triggered after Bridge synchronizes a route. React and Vue consumers report
host-to-remote `popstate` synchronization, while a Vue producer also reports
initial memory-route navigation. `operation.result` contains the router result
when the framework provides one.

```ts
function afterBridgeRouteSync(operation: BridgeOperationResult): void;
```

```ts
import type { ModuleFederationRuntimePlugin } from '@module-federation/enhanced/runtime';

const bridgeTracePlugin = (): ModuleFederationRuntimePlugin => ({
  name: 'bridge-trace-plugin',
  afterBridgeRender(_info, operation) {
    console.log('bridge render', operation?.context);
  },
  afterBridgeRouteSync(operation) {
    console.log('bridge route', operation.context.route);
  },
});
```
