...
If you only need to **register** a plugin path in build config, see [`runtimePlugins`](/configure/runtimeplugins.html). If you need the full hook list, see [Runtime Hooks](/guide/runtime/runtime-hooks.html).

## Mental model

A runtime plugin is a function that returns a `ModuleFederationRuntimePlugin`.
New plugins should return their hooks from `apply(instance)`:
...
## Access the runtime instance in a plugin

Previously, only a few hooks provided the runtime instance as an argument. If a
plugin needs to access its runtime instance from any hook, implement
`apply(instance)`. It is called once per plugin name on each instance, where the
plugin can access the current instance and return the hooks to register for it.

```ts
import type { ModuleFederationRuntimePlugin } from '@module-federation/enhanced/runtime';

const instanceTracePlugin: ModuleFederationRuntimePlugin = {
  name: 'instance-trace-plugin',
  apply(instance) {
    ...
    return {
      afterLoadRemote(args) {
        completedLoads += 1;
        console.log(instance.name, args.id, completedLoads);
      },
    };
  },
};
...
The `instance` passed to `apply(instance)` is the current runtime instance. Use
it to read instance information or call instance methods. Because each instance
calls `apply` separately, counters, caches, and subscriptions declared there
also remain isolated. The returned object is the hook set registered for that
instance.

### Legacy syntax and compatibility

Previously, hooks could be declared directly on the plugin object:

```ts
const legacyPlugin: ModuleFederationRuntimePlugin = {
  name: 'legacy-plugin',
  beforeRequest(args) {
    return args;
  },
...
This syntax is still supported, but will be deprecated. To migrate, move the
existing hooks into the object returned by `apply(instance)`. When `apply`
returns nothing, the current version still falls back to hooks declared on the
plugin object.
...
| Observe remote load status                       | `afterMatchRemote`, `afterLoadRemote`               | You need request tracing or final success/failure status for `loadRemote`                                                                                             |
| Observe shared dependency status                 | `afterRegisterShare`, `afterLoadShare`, `errorLoadShare` | You need to diagnose provider registration, missing shared dependencies, version mismatch, or eager configuration errors                                         |
| Observe manifest loading                         | `beforeLoadManifest`, `afterLoadManifest`           | You need manifest timing, cache, validation, or recovery diagnostics                                                                                                  |
| Observe Bridge operations                        | `beforeBridgeRender`, `afterBridgeRender`, `beforeBridgeDestroy`, `afterBridgeDestroy`, `afterBridgeRouteSync` | You need framework, producer/consumer, render, teardown, or route synchronization context                              |
...
- [`runtimePlugins` config](/configure/runtimeplugins.html)
