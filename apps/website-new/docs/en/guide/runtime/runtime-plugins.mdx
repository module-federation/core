...
## Create hooks for each runtime instance

A plugin can implement `apply(instance)` when its hook handlers need state that
belongs to one specific Module Federation instance. `apply` is called once per
plugin name on each instance and may return the hooks to register for that
instance.
...
Use this form when the same global plugin object is shared by multiple runtime
instances but its counters, caches, or subscriptions must stay isolated. When
`apply` returns hooks, that returned object is the hook set registered for the
instance; include every hook that instance needs in the returned object.
