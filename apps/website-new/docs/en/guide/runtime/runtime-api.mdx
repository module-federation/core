...
```ts
import { createInstance, getRemoteInfo, getRemoteEntry } from '@module-federation/enhanced/runtime';
...
- ShareArgs parameters:
  - lib: `() => Module` synchronous factory; should not return Promise.
  - get: `() => () => Module` or `() => Promise<() => Module>`, for lazy/async loading.
- Others: scope, shareConfig, deps are optional; version is recommended for distinguishing.
