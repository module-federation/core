...
interface RemoteInfoCommon {
  ...
  // Custom request options (e.g. `headers`) used when loading this remote.
...
### Loading remotes with custom headers (`fetchOptions`)

`fetchOptions` (a standard [`RequestInit`](https://developer.mozilla.org/en-US/docs/Web/API/RequestInit)) attaches custom HTTP options — most commonly `headers` such as `Authorization` — to the requests used to load a remote. It can be set per remote, or at the call level where it applies to every remote in that call. A remote's own `fetchOptions` is merged on top of the call-level one, and its `headers` win on conflict.

This is supported for **`module` / `esm`** remotes: their manifest, remote entry, split chunks, shared dependencies, and manifest-declared CSS are loaded through a `fetch` + blob loader so the headers reach every asset request. Remotes without `fetchOptions` (and non-ESM remotes) are loaded as before — note that `<script>` / `<link>`-based loading cannot carry custom headers.
...
// Call level — applies to every remote in this call
mf.registerRemotes(
  [{ name: 'sub2', entry: 'https://localhost:2002/mf-manifest.json', type: 'module' }],
  { fetchOptions: { headers: { Authorization: `Bearer ${token}` } } },
);
```
...
Native preload hints (`<link rel="modulepreload">` / `<link rel="preload">`) cannot carry headers, so they may fail on an authenticated origin; the actual module loads still go through the authenticated `fetch` loader. Authenticated CSS is injected directly and does not emit the `createLink` hook, so plugin-added `nonce`/SRI attributes are not applied to it.
