...
### Loading remotes with custom headers (the `fetch` hook)

Custom HTTP options — most commonly `headers` such as `Authorization` — are request behavior, so they are configured through the runtime [`fetch` hook](./runtime-hooks#fetch) rather than on remote registration. The hook receives the request URL, its `RequestInit`, and the target `remoteInfo`, so you can decide headers per remote and handle dynamic cases such as token refresh or per-request auth logic.

For **`module` / `esm`** remotes, when a `fetch` hook is registered the remote entry and its manifest-declared CSS are loaded through a `fetch` + blob loader so the headers you add reach every asset request. The manifest request already flows through this hook. With no `fetch` hook registered, remotes load as before via native `import()` / `<link>` (which cannot carry custom headers).
...
const mf = createInstance({
  ...
  plugins: [
    {
      name: 'auth-fetch',
      // `remoteInfo` lets you scope headers to specific remotes
...
Native preload hints (`<link rel="modulepreload">` / `<link rel="preload">`) cannot carry headers, so they may fail on an authenticated origin; the actual module loads still go through the `fetch` hook. Authenticated CSS is injected directly and does not emit the `createLink` hook, so plugin-added `nonce`/SRI attributes are not applied to it.
