...
- **`getKey`** only overrides the "arguments" portion of the key — **the function itself still participates in the cache-key computation**. Two different functions whose `getKey` returns the same value will not share the cache.
- **`customKey`** returns the **entire final cache key**, replacing the framework's default "function + arguments" key. It can therefore let **different functions share the same cache**.
...
- `fn`: The original cached function.
