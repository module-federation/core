...
:::tip Runtime API imports
The following examples import Runtime APIs from `@module-federation/enhanced/runtime` by default. Modern.js projects can import from `@module-federation/modern-js-v3/runtime` or `@module-federation/modern-js/runtime` instead. For other integrations, see [How to import Runtime APIs](/integrations#how-to-import-runtime-apis).
:::
...
By default, parameters are passed to the loader function. The type is [DataFetchParams](/guide/bridge/react/load-component#datafetchparams), which includes the following field:
