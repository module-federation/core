...
| An AI application's embedded browser that supports Chrome extensions, such as Codex's embedded browser | Browser (`dist/browser`) | Compact popup opened from the extension icon             |

Both variants provide the same debugging features: **module proxying and version switching, module information, dependency graphs, shared dependencies, React HMR / Fast Refresh, Loading Trace, and report export**, and both support WebMCP. WebMCP tool discovery and invocation have been verified in Codex's embedded browser.
...
2. Extract the ZIP and load the extracted folder in the target embedded browser's extension manager.
