...
In {props.brandName || props.name || 'Module Federation'}, the main global variables are `__FEDERATION__`, `__SHARE__` and `__INSTANCES__`. They record the module federation info, shared module info, and instance info of the current application, and help us understand and debug the runtime behavior of module federation.
...
:::tip
It is recommended to use [chrome devtools](/guide/debug/chrome-devtool.html) to view the processed `moduleInfo`.
:::
...
- `moduleName` is the name of the module, and `moduleVersion` is the version of the module or its `url`.

- `moduleVersion` only exists for producer modules.
...
1. `react` has only one version, `18.3.1`.
2. Check `__SHARE__["app1:0.1.94"].default.react["18.3.1"]` — `loaded` is `true`, indicating the module has been loaded.
3. The `from` field is `webpack_provider`, meaning the `react` module was provided by that module.
4. Finally, look at the `useIn` field: it contains `webpack_provider` and `app1`, meaning this version of `react` is consumed by both modules.
