...
## Divebell Integration

Enable Divebell synchronization when an agent or browser tool needs stable
targets, waitable statuses, and safe actions instead of reading raw reports
directly:
...
`divebell: true` reuses the Divebell runtime installed on `window`, or creates
one when needed. The object form can also provide a custom `runtime` or browser
`host`. Divebell output is diagnostic-only: registration or synchronization
errors do not change Module Federation loading results.

The plugin registers these targets:

| Target | Type | Statuses | Purpose |
| --- | --- | --- | --- |
| `mf:remote:<remoteName>` | `mf.remote` | `registered`, `loading`, `ready`, `error`, `recovered` | Overall state of one remote. |
| `mf:remote:<remoteName>:expose:<exposeName>` | `mf.remote.expose` | `registered`, `loading`, `ready`, `error`, `recovered` | Wait for one exposed module to become usable. |
| `mf:shared:<sharedName>:<version>:<shareScope>` | `mf.shared` | `unloaded`, `loading`, `loaded`, `recovered`, `error` | Inspect one observed shared dependency entry. |
| `mf:shared-conflict:<sharedName>:<shareScope>` | `mf.shared.conflict` | `warning` | Diagnose multiple versions of a singleton shared dependency. |

Remote, expose, and shared snapshots include a stable `instanceRef`. Use it to
separate reports and runtime state when more than one Module Federation instance
exists on the page. Manifest and remoteEntry details remain phases on the
remote/expose data; the plugin does not create separate manifest or remoteEntry
targets.
...
Safe actions registered by the plugin:
...
`mf:list-reports` accepts `traceId`, `instanceRef`, `remote`, `expose`,
`shared`, `status`, `outcome`, and `limit` filters.

The browser reader's `getRuntimeState()` method and the
`mf:get-runtime-state` action return the same safe state model. It includes
stable instance references, roles and evidence, remotes, loaded producers,
share-scope summaries, Bridge state, consumer-to-producer relationships,
deployment `moduleInfo`, and capability completeness. Prefer `instanceRef`
over array indexes when selecting an instance.
...
| `divebell` | `boolean \| DivebellObservabilityOptions` | `false` | Synchronize reports into Divebell targets and register safe actions. `true` uses the default browser host/runtime. |
| `divebell.enabled` | `boolean` | `true` | Disable synchronization while retaining a shared Divebell options object. |
| `divebell.source` | `string` | `'module-federation'` | Source attached to registered Divebell targets, snapshots, and actions. |
| `divebell.runtime` | `DivebellCore` | auto | Use an existing Divebell runtime instead of resolving or creating one. |
| `divebell.host` | `DivebellWindowHost` | `window` | Browser host used to resolve or install the Divebell runtime. |
