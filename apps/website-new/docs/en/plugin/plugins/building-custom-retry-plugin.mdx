...
Test your retry plugin with various failure scenarios:
...
describe('Custom Retry Plugin', () => {
  ...
  it('should handle network failures with retry', async () => {
    ...
    const mf = init({
      name: 'test-host',
      remotes: [
        { name: 'remote1', entry: 'http://localhost:3001/mf-manifest.json' }
      ],
      plugins: [plugin]
    });

    // Should retry and eventually succeed
    const module = await mf.loadRemote('remote1/module');
  ...
  it('should activate circuit breaker after threshold', async () => {
    ...
    // First attempts should try to load
    try {
      await mf.loadRemote('remote1/module1');
    } catch (e) {}
    
    try {
      await mf.loadRemote('remote1/module2');
    } catch (e) {}

    // Circuit should be open now - should return fallback immediately
    const start = Date.now();
    const fallback = await mf.loadRemote('remote1/module3');
    const duration = Date.now() - start;
    
    expect(duration).toBeLessThan(100); // Should not retry
    expect(fallback.default).toBeDefined(); // Should return fallback
  });

  it('should render fallback component correctly', async () => {
    const plugin = customRetryPlugin({
      retryAttempts: 1,
    });
    ...
    const module = await mf.loadRemote('remote1/Component');
    const FallbackComponent = module.default;
    
    // Test that fallback component is returned
    expect(FallbackComponent).toBeDefined();
    expect(typeof FallbackComponent).toBe('function');
    
    // Render and check content
    const rendered = await FallbackComponent();
    expect(rendered.type).toBe('div');
    expect(rendered.props.style.backgroundColor).toBe('#fff2f0');
  });
  ...
  it('should handle timeout scenarios', async () => {
    ...
    expect(module.default).toBeDefined(); // Should return fallback
  });
});
```
