...
- `@module-federation/metro-plugin-rock` - [Rock](https://www.rockjs.dev/) integration
- `@module-federation/metro-plugin-rnef` - RNEF integration (deprecated, use `metro-plugin-rock` instead)
...
For projects using [Rock](https://www.rockjs.dev/) (formerly RNEF), add the Module Federation plugin to your `rock.config.mjs`:
...
> **Note**: If you are migrating from RNEF, replace `@module-federation/metro-plugin-rnef` with `@module-federation/metro-plugin-rock` and rename `rnef.config.mjs` to `rock.config.mjs`. See the [metro-plugin-rock README](https://www.npmjs.com/package/@module-federation/metro-plugin-rock) for full migration details.
