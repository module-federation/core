...
new NextFederationPlugin({
  ...
  extraOptions: {
    ...
    scriptTimeout: number // `8000` by default
  },
});
```
...
- `scriptTimeout` – how long (in milliseconds) the browser waits for a remote entry script before the remote is reported as unavailable. Increase it if your users are on slow networks.

Next.js derives the expose asset fields used by legacy `flushChunks()` from `mf-manifest.json` without an additional Webpack stats traversal, so applications on older versions continue to work during rolling upgrades. Custom tooling that reads other legacy stats fields should migrate to `mf-manifest.json`; setting `manifest: false` retains the complete legacy output.
