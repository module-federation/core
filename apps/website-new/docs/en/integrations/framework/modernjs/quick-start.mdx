...
This chapter explains how to use the Module Federation plugin in Modern.js and provides a simple example that demonstrates how to create provider and consumer applications.
...
Before getting started, you will need to install [Node.js](https://nodejs.org/), and ensure that your Node.js version is no lower than v20.19.5. **We recommend using the LTS version of Node.js 24.**
...
Here is an example of how to install the Node.js 24 LTS version via nvm:
...
Module Federation provides a matching plugin `@module-federation/modern-js-v3` for Modern.js.
...
Start the project and visit `http://localhost:3007/` and find that SSR is working normally and the page can be rendered normally, but there will be a problem of style flickering. This is because the producer's style files cannot be injected into the corresponding HTML. This issue can be resolved with [createLazyComponent](/guide/bridge/react/load-component.html#what-is-createlazycomponent).

Modify the consumer code that references the producer:
