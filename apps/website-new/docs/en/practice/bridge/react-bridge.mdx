...
- **React 16**: Fully supported, using traditional `ReactDOM.render` and `ReactDOM.hydrate` APIs
- **React 17**: Fully supported, maintaining the same API calling pattern as React 16
- **React 18**: Fully supported, automatically using the new `createRoot` and `hydrateRoot` APIs
- **React 19**: Fully supported, implemented through dynamic import of `react-dom/client`, ensuring it works even in older environments
