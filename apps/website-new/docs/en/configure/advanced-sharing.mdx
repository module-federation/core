...
```
In this scenario, if no suitable remote version is found, the system will refuse to use the fallback (which is version 17.0.2) since it matches the exclude pattern. This prevents runtime errors that would occur from loading incompatible React versions.
...
new ModuleFederationPlugin({
  ...
  shared: {
    ...
    'moment': {
      ...
      // Result: The local fallback './local-moment-v2.10' will NOT be used,
