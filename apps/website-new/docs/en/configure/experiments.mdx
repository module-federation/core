...
interface ExperimentsOptions {
  /**
   * Enable external runtime loading.
   * When true, uses the external MF runtime provided by the consumer.
   * @default false
   */
  externalRuntime?: boolean;
  
  /**
   * Provide external runtime for other modules.
   * Only configure on the topmost consumer to avoid conflicts.
   * @default false
   */
  provideExternalRuntime?: boolean;
  
  /**
   * Enable asynchronous startup for all Module Federation entrypoints.
   * Eliminates the need for manual import() statements.
   * @default false
   */
  ...
  optimization?: {
    /**
     * Disable snapshot plugin to reduce bundle size.
     * WARNING: Disables mf-manifest protocol, TypeScript syncing, and HMR.
     * @default false
     */
     ...
     * Affects runtime optimizations and tree-shaking.
     * @default Inferred from Webpack's target option
     */
    target?: 'web' | 'node';
  };
}
...
const config: ModuleFederationPluginOptions = {
  name: 'remote',
  experiments: {
    externalRuntime: true, // Use external runtime
    optimization: {
      disableSnapshot: true, // Reduce bundle size
      target: 'web' // Web-specific optimizations
    }
  }
};
```

### Host with External Runtime Provider

```ts
const config: ModuleFederationPluginOptions = {
  name: 'shell',
  experiments: {
    provideExternalRuntime: true, // Provide runtime to others
    asyncStartup: true,
    optimization: {
      target: 'web'
    }
