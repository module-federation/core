...
### disableRemote and disableShared

- Type: `boolean`
- Required: No
- Default: `false`

These options remove independent runtime-core capabilities at build time:

- `disableRemote` removes the complete remote consumption path, including registration, entry loading, preloading, reading and executing exposed modules, and related snapshot handling. It does not remove the `exposes` or `container.get` generated for the current producer. Use it only when the build never consumes or dynamically registers remotes.
- `disableShared` removes shared dependency registration, resolution, and loading. A minimal empty share scope is retained so a remote without shared dependencies can still initialize.

The options can be combined. Calling an API for a disabled capability throws an explicit error. Keep every option at its default unless the build is guaranteed not to use that capability.

### target

- Type: `'web' | 'node'`
- Required: No
- Default: Not set; not inferred from Webpack or Rspack's `target` option

This option defines the `ENV_TARGET` global constant during the build, specifying the intended execution environment. It is only applied when explicitly set; the plugin does not infer it from the bundler's `target` option.
...
Explicitly setting this value is recommended to ensure the intended optimizations are applied. For web builds, set `target: 'web'` to enable compile-time elimination of Node.js-specific loader code; set `target: 'node'` for Node.js builds, especially in universal or server-side rendering scenarios.
