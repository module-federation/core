...
### disableRemote, disableExpose, and disableShared
...
- `disableRemote` removes remote registration, loading, preloading, and related snapshot handling. Use it only when the build never consumes or dynamically registers remotes.
- `disableExpose` removes the code that reads and executes exposed modules from a loaded remote. Remote entries can still be initialized for shared dependency negotiation.
