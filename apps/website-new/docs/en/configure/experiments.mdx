...
`FEDERATION_OPTIMIZE_*` and `ENV_TARGET` still select the default runtime modules. A later major version can drop these defines after every supported bundler passes `module-federation:*` package conditions.

The packages also accept `module-federation:target-web`, `module-federation:target-node`, `module-federation:target-worker`, and `module-federation:target-universal`. Compiler plugins do not set those conditions when the family mode is `legacy-defines`.
