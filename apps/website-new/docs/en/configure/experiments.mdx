...
`FEDERATION_OPTIMIZE_*` and `ENV_TARGET` still control the default runtime leaves. A future major release can remove these defines after every supported bundler uses `module-federation:*` package conditions.

The package contract reserves `module-federation:target-web`, `module-federation:target-node`, `module-federation:target-worker`, and `module-federation:target-universal`. Compiler plugins do not add these conditions while the selected family reports `legacy-defines`.
