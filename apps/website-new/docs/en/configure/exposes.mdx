...
The `exposeKey` is essentially consistent with the [Package Entry Points](https://nodejs.org/api/packages.html#package-entry-points) specification (except for not supporting regular expression matching). Example usage:
