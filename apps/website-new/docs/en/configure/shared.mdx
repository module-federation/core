...
interface SharedObject {
  [sharedName: string]: SharedStrategy | SharedConfig;
}

type SharedStrategy = 'loose';
...
Currently, only one sharing strategy is supported:

- **loose**: Loose mode, which defaults to reusing shared dependencies that match the current dependency version range. If the version specification is not met, a higher version of the dependency will be used, with a warning in the console.
  The version range is taken as:

  - `^${require('project/package.json')[devDeps | dep]['depName']}`

  The corresponding configuration for this strategy is:

  ```tsx
  {
    singleton: true,
    requiredVersion: require('project/package.json')[devDeps | dep]['depName']
  }
  ```
