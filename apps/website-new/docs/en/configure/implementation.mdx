...
The path must resolve one complete runtime family. If a package in that family is missing, the compiler does not fill it from the copy installed next to the bundler plugin.

`runtime`, `runtime-core`, `webpack-bundler-runtime`, and `sdk` must resolve from the dependency graph of the chosen `runtime-tools` path.

Vite and Rolldown cannot apply different resolver rules per importing file. A Vite setup that uses package conditions must use one family for the whole process and fail if a second family appears. The esbuild adapter in this repo still imports the full `@module-federation/webpack-bundler-runtime` entry.
