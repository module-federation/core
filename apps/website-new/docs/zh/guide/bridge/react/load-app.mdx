...
默认情况下，Bridge 会启用路由支持，以提供最佳的开发体验和兼容性。当你设置 `enableBridgeRouter: false` 时，插件会自动切换到一个不包含 `react-router-dom` 的版本，从而优化 bundle 体积。

:::info 工作原理

插件会自动将 `@module-federation/bridge-react` 的导入解析到 `@module-federation/bridge-react/base`。这个导出中不包含 `react-router-dom` 的入口产物，因此用户无需任何修改。
...
// 无需修改其他代码。如果远程模块依赖 basename，则手动传递。
<RemoteApp basename="/" />
```
