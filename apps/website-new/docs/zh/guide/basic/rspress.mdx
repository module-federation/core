...
当使用 [@rspress/plugin-llms](https://v2.rspress.rs/zh/plugin/official-plugins/llms) 插件，并且开启了 `mdfiles` ，Rspress 会自动将源文件内容抽取并移动到输出目录。

但是该插件无法处理通过 MF 获取的文档组件场景。

为了处理此问题，MF Rspress Plugin 会在 SSG 完成后根据渲染完成的 `html` 重新生成 `.md` 文件，使 `llms` 可以正常获取到文档内容。
