...
`Federation Runtime` 是新版本 `Module Federation` 的主要功能之一，它能够支持在运行时注册共享依赖、动态注册和加载远程模块，还可以通过 `Plugin` 来扩展 `Module Federation` 在运行时的能力，构建插件和 Runtime 一起使用，构建插件是基于 Runtime 的基础实现的。了解 Runtime 内容可以参考：[Why Runtime](#why-runtime)。
...
<PackageManagerTabs
  command={{
    npm: 'npm add @module-federation/runtime --save',
    yarn: 'yarn add @module-federation/runtime --save',
    pnpm: 'pnpm add @module-federation/runtime --save',
    bun: 'bun add @module-federation/runtime --save',
  }}
/>
...
**Info**: 只有当 entry 是 manifest 文件协议时，才能加载更多的资源。
...
- 在现有项目中导出模块的非常低，无需安装过多额外的依赖和构建配置，只需要声明模块名称和导出的模块路径就可以完成模块导出
