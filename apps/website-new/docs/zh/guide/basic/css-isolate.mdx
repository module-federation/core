...
在 Module Federation 中，宿主和远程应用运行在同一个页面、共享同一份全局 CSS 作用域，一方的样式很容易意外影响到另一方，造成样式污染。Module Federation 本身不内置样式隔离能力（原因见下方 [FAQ](#为什么-module-federation-不直接处理-css-样式隔离)），建议在生产者侧按场景选择合适的方案。

## BEM（Block Element Modifier）

BEM 是一种命名规范，通过给类名添加前缀和后缀来实现样式隔离。BEM 的结构如下：
...
CSS Modules 通过将每个CSS文件作为一个模块来实现样式隔离，每个类名在编译时会被转换成唯一的标识符。[Rsbuild 中使用 css module](https://v2.rsbuild.rs/zh/guide/styling/css-modules)
...
CSS-in-JS 将样式直接写在 JavaScript 文件中，常用的库有 [styled-components](https://styled-components.com/) 、[emotion](https://github.com/emotion-js/emotion) 等。[Rsbuild 中使用 css-in-js](https://rsbuild.rs/zh/guide/styling/css-in-js)
...
Shadow DOM 是 Web 组件技术的一部分，通过创建独立的DOM树来实现样式隔离。React 可以通过 [react-shadow](https://www.npmjs.com/package/react-shadow) 使用 ShadowDOM。
