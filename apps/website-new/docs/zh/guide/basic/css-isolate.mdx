...
CSS Modules通过将每个CSS文件作为一个模块来实现样式隔离，每个类名在编译时会被转换成唯一的标识符。[Rsbuild 中使用 css module](https://v2.rsbuild.rs/zh/guide/styling/css-modules)
...
CSS-in-JS将样式直接写在JavaScript文件中，常用的库有 [styled-components](https://styled-components.com/) 、[emotion](https://github.com/emotion-js/emotion) 等。[Rsbuild 中使用 css-in-js](https://rsbuild.rs/zh/guide/styling/css-in-js)
