...
若使用构建插件，项目启动时将自动创建 MF 实例并存储于内存中。此时可直接通过 <Badge>API</Badge> 调用 MF 实例的方法。

若未使用构建插件，则需手动创建 MF 实例，之后调用相应 API。
...
## init <Badge>API</Badge> <Badge type='danger'>废弃</Badge>

:::warning Warning

此 API 将被废弃。如果需要获取已创建的实例，可以使用 [getInstance](#getinstance) 获取已创建的实例。

如果需要创建新的实例，请使用 [ModuleFederation](#modulefederation) class。
