...
`runtime-infer` 策略会基于运行时已加载的模块进行判断，虽然灵活性高，但由于缺少完整的依赖信息，复用率会有所降低。

因此需要用户手动添加额外的 [usedExports](../../configure/shared#usedexports) 来提高 shared 的复用率。


### 策略二：服务端辅助策略 (`server-calc`)
...
4. 更新 snapshot 中的 `secondarySharedTreeShakingEntry`、`secondarySharedTreeShakingName`和 `treeShakingStatus` 字段，并下发数据。
...
注意：需要关注的是如果 mode 为 `runtime-infer` 或者在运行时加载动态生产者，那么可能会发生「项目 A 加载**自身** Tree Shaking 的共享依赖，项目 B 加载**自身**全量的共享依赖」的情况。如果这个共享依赖是必须单例，那么可能会出现页面崩溃或未知情况。因此如果是必须代理的组件库，请小心设置！
