...
可以看到生产者的请求被提前到跟部分 js 并行了，**目前在最没有前置操作的简单场景中测试首屏可以提升40～100ms，次屏理论上可以完全做到直出**，在前置流程长的情况下可以大大提升模块整体的渲染速度
...
export const Button = () => {
  const [userInfoPrefetch, reFetchUserInfo] = usePrefetch<UserInfo>({
    // 对应生产者 vmok.config.ts 中的 (name + expose)，例如 `@vmok-e2e/edenx-demo-app2/Button` 用于消费 `Button.prefetch.ts`
...
### preloadRemote
#### 功能
用户如果在消费者项目中手动调用 [preloadRemote](/guide/basic/runtime.html#preloadremote) API 并且设置参数 `prefetchInterface` 为 `true`，那么会认为消费者不仅希望提前加载生产者的静态资源，同时希望将接口请求也提前发出，这可以让项目获得更快的渲染速度,
这在**首屏有前置请求**场景或者**希望次屏直出**这些场景中尤其有效
#### 使用方法
``` ts
import { preloadRemote } from '@vmok/kit/runtime';

preloadRemote([
  {
    nameOrAlias: '@vmok-e2e/edenx-demo-app2',
    // 开启接口预请求
...
### 9. Vue 或其他框架想用怎么办
我们提供了通用的 Web API，只是未在 Vue 等其他框架中提供类似 `usePrefetch` 这种 hook（后续会支持），可以联系我们针对非 React 框架先在业务中使用起来
