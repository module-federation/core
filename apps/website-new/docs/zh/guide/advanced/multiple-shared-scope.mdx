...
* **生产者**：用 [shareScope](/configure/shareScope.html) 声明该 Provider 会初始化哪些共享池（默认 `default`，支持 `string | string[]`）。
* **消费者**：用 [remotes[remote].shareScope](/configure/remotes.html#sharescope) 声明与某个 Consumer 需要对齐哪些共享池（默认 `default`）。
* **shared 条目**：用 `shared[pkg].shareScope` 决定某个依赖注册 / 解析在哪个共享池（见 [shared.shareScope](/configure/shared.html#sharescope)）。
