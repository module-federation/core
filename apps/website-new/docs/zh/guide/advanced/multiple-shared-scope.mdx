...
`shareScope` / `remotes[remote].shareScope` 虽然都支持 `string | string[]`，但不要随意用数组，尤其不要写成 `['default']` 或 `[]`：

* **单个 scope**：请用字符串（例如 `'default'`），不要写成 `['default']`。根据运行时实现，`'default'` 与 `['default']` 在“共享池对齐/初始化”的分支上是不同的：当 host 配的是数组而 remote 配的是字符串时，remote 会按 host 的列表去对齐 scope；而 remote 配成数组时，则只会按 remote 的列表处理。
...
## 示例：React 17 与 React 18 共存

下面以 [multiple-share-scope 示例](https://github.com/module-federation/module-federation-examples/tree/master/multiple-share-scope) 为例，演示如何让同一个 host 同时加载基于 React 17 的 remote 和基于 React 18 的 remote，二者的 React 实例完全隔离互不干扰。

**应用结构：**

| 应用 | 端口 | 说明 |
| --- | --- | --- |
| `host` | 8080 | 宿主应用，同时消费两个 remote |
| `provider-react-17` | 8081 | 使用 React 17 构建的 remote |
| `provider-react-18` | 8082 | 使用 React 18 构建的 remote |

**共享池设计：**

* `default` scope — 承载 React 17（`react17` / `react-dom17` 包别名）
* `react18` scope — 承载 React 18（`react` / `react-dom`）
...
`provider-react-17` 只使用默认 scope，无需额外配置 `shareScope`：
...
`provider-react-18` 需要独立的 `react18` scope 来隔离自己的 React 18 实例：
...
* `shareScope: ['react18', 'default']` 声明该 remote 会初始化 `react18` 和 `default` 两个共享池。
* `shared[pkg].shareScope: 'react18'` 把 react/react-dom 注册到 `react18` 独立池中，不会与 `default` 池中的 React 17 发生版本合并或 singleton 冲突。

### 消费者（host）

host 安装了 `react17` / `react-dom17`（即 `npm:react@^17` 别名）和正式的 `react` / `react-dom`（React 18），并分别放入不同 scope：
...
export default createModuleFederationConfig({
  ...
  remotes: {
    provider17: {
      ...
      // 未配置 shareScope，默认使用 'default' — 与 provider17 对齐
    },
    provider18: {
      external: 'provider18@http://localhost:8082/mf-manifest.json',
      shareScope: ['react18', 'default'], // 把 react18/default 两个 scope 传入 provider18
    },
  },
  shared: {
    // React 18：注册到 react18 scope
    react: {
      singleton: true,
      shareScope: 'react18',
    },
    ...
    // React 17：以包别名注册到 default scope
    react17: {
      shareKey: 'react',
      request: 'react17',
      shareScope: 'default',
...
* `remotes[remote].shareScope` 决定 host 在 init 这个 remote 时，会把哪些 scope 的 shareScopeMap 传入 remote。
* `provider17` 不配置 `shareScope`，默认走 `default` 池，只能拿到 React 17。
* `provider18` 配置 `shareScope: ['react18', 'default']`，host 会把 `react18` 和 `default` 两个 scope 都传给它，`provider18` 的 React 18 shared 因此能从 host 的 `react18` 池复用。
* `shareKey` / `request` 用于让 `react17` 包以 `react` 这个 shareKey 参与共享，provider17 在 `default` scope 解析 `react` 时就会匹配到这个条目。
...
registerShared({
  ...
  // React 17 以 shareKey='react' 注册到 default scope
  react17: {
    shareKey: 'react',
    version: '17.0.2',
    scope: 'default',
