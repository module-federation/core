...
### 不使用 ModuleFederationPlugin 时按需组合 Runtime

如果项目不使用 `ModuleFederationPlugin`，而是直接打包 Runtime，请从 `@module-federation/runtime/compose` 只导入项目用到的能力。没有导入的能力不会进入模块图，与优化模式无关。

例如，一个只消费 Remote 的 Web Host：
...
构建需要时，再从 `@module-federation/runtime-core/shared` 导入 `shared`，从 `@module-federation/runtime-core/snapshot` 导入 `snapshot`。`import { init } from '@module-federation/runtime'` 仍然提供完整的 Runtime。

:::info
`FEDERATION_OPTIMIZE_NO_REMOTE`、`FEDERATION_OPTIMIZE_NO_SHARED`、`FEDERATION_OPTIMIZE_NO_SNAPSHOT_PLUGIN` 和 `FEDERATION_HAS_EXPOSES` 已被移除，Runtime 不再读取它们。
:::
