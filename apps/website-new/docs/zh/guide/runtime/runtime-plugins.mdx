...
如果你需要完整 Hook 列表，见 [Runtime Hooks](/guide/runtime/runtime-hooks.html)。

## 心智模型

一个 Runtime 插件本质上是一个返回 `ModuleFederationRuntimePlugin` 的函数。新插件应通过
`apply(instance)` 返回需要注册的 hook：
...
过去只有少数 hook 会通过参数提供 runtime 实例。如果插件需要在任意 hook 中访问其所属的
runtime 实例，可以实现 `apply(instance)`。每个实例会按插件名调用一次 `apply`，插件可以
在这里获取当前实例，并返回为该实例注册的 hook。
...
`apply(instance)` 中的 `instance` 就是当前 runtime 实例，可以用来读取实例信息或调用实例
方法。由于每个实例都会单独调用一次 `apply`，在其中声明的计数器、缓存或订阅也会相互
隔离。`apply` 的返回对象是该实例最终注册的 hook 集合，因此要把该实例需要的 hook 都放进
返回对象。
...
这种写法目前仍然兼容，但后续会废弃。迁移时把原有 hook 移入 `apply(instance)` 的返回
对象即可；如果 `apply` 没有返回内容，当前版本仍会回退到插件对象上的 hook。
...
| 观测 remote 加载状态               | `afterMatchRemote`、`afterLoadRemote`               | 需要记录 `loadRemote` 的请求链路或最终成功/失败状态                                                                       |
| 观测 shared 依赖状态               | `afterRegisterShare`、`afterLoadShare`、`errorLoadShare` | 需要诊断提供方注册、shared 缺失、版本不匹配、eager 配置错误等问题                                                    |
| 观测 manifest 加载                 | `beforeLoadManifest`、`afterLoadManifest`           | 需要记录 manifest 耗时、缓存、校验或恢复情况                                                                              |
| 观测 Bridge 操作                   | `beforeBridgeRender`、`afterBridgeRender`、`beforeBridgeDestroy`、`afterBridgeDestroy`、`afterBridgeRouteSync` | 需要获取框架、生产方/消费方、渲染、销毁或路由同步上下文                    |
| 加载失败时兜底                     | `errorLoadRemote`                                   | 需要离线兜底、fallback module、分层恢复策略                                                                               |
| 扩展新的 remote 加载方式           | `loadEntry`                                         | 需要完整接管 remote entry 加载过程，或实现新的 remote 类型                                                                |

## 示例：改写解析后的 remote entry
...
- [`runtimePlugins` 配置](/configure/runtimeplugins.html)
- [Runtime API](/guide/runtime/runtime-api.html)
- [Runtime Hooks](/guide/runtime/runtime-hooks.html)
- [插件系统](/plugin/dev/index.html)
- [Retry 插件](/plugin/plugins/retry-plugin.html)
- [Error Load Remote 的解决方案](/blog/error-load-remote.html)
