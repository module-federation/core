...
## 为每个 runtime 实例创建独立 hook

当 hook 处理函数需要保存某个 Module Federation 实例自己的状态时，插件可以实现
`apply(instance)`。每个实例会按插件名调用一次 `apply`，它可以返回只为这个实例注册的
hook。
...
同一个全局插件对象被多个 runtime 实例共用，但计数器、缓存或订阅不能互相影响时，
可以使用这种写法。`apply` 返回 hook 后，这个返回对象就是该实例最终注册的 hook
集合，因此要把该实例需要的 hook 都放进返回对象。
