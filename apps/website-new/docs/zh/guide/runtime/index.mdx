...
- 如果你的项目是 Modern.js 项目且使用 `@module-federation/modern-js-v3`，运行时应该从 `@module-federation/modern-js-v3/runtime` 中导出运行时 API。这样能保证插件和运行时使用的是同一个运行时实例，保证模块加载正常

- 如果你的项目是 Modern.js 项目但是没有使用 `@module-federation/modern-js-v3`，则应当从 `@module-federation/enhanced/runtime` 导出 runtime API。但是我们推荐你使用 `@module-federation/modern-js-v3` 进行模块注册和加载，这将使你享受到更多和框架结合的能力。
