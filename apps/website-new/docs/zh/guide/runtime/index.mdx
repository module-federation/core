...
默认项目安装 `@module-federation/enhanced`，并从 `@module-federation/enhanced/runtime` 引用 Runtime API。Modern.js 项目如果已经使用 `@module-federation/modern-js-v3`，应该从 `@module-federation/modern-js-v3/runtime` 引用 Runtime API，保证框架插件和手动调用的 Runtime 使用同一个实例。
...
- [Runtime API](./runtime-api)：查看 `createInstance`、`loadRemote`、`registerRemotes` 等 API。
- [Runtime 插件](./runtime-plugins)：了解如何扩展运行时加载流程。
- [Runtime Hooks](./runtime-hooks)：查看插件可使用的生命周期。
