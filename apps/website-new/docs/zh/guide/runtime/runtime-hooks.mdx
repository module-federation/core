...
在运行时尝试把某个 shared 版本注册到共享池后触发。这个 hook
会同时给出候选记录和最终保留下来的记录，因此也能观察“已有提供方胜出”的冲突场景。
...
- `shared` 是运行时本次尝试注册的候选记录。
- `previousShared` 是注册前相同 scope、包名和版本下的记录。
- `registeredShared` 是冲突处理后最终保留的记录。
- `trigger` 表示这次注册来自构建生成的运行时代码、运行时 API，还是 remote
  container 初始化。
...
`loadContext` 会在信息可用时标明由哪个消费方触发了 shared 解析：
...
因为 `beforeLoadShare` 是 waterfall hook，插件可以返回更新后的
`loadContext`；同一个值随后会继续传给 `resolveShare`、`afterLoadShare`
和 `errorLoadShare`。
...
在运行时请求并解析 remote manifest 之前立即触发。manifest
已经存在于运行时缓存中时不会触发。
...
manifest 加载结束后触发。网络加载成功、命中缓存、校验失败，以及由
`errorLoadRemote` 恢复的情况都会触发。
...
- `cached` 为 `true` 时表示没有发起网络请求。
- `error` 保留原始的加载错误或校验错误。
- `recovered` 为 `true` 时表示 `errorLoadRemote` 返回了可用的 manifest。
...
`resourceContext` 用于判断这次资源加载来自 `loadRemote`、`preloadRemote`
还是 shared 初始化，以及对应的资源类型、id 和可用时的 exposed module。
...
`fetch` 函数允许自定义加载 remote 时使用的请求。它始终会作用于清单（manifest）JSON 请求（成功的 `Response` 必须返回一个有效的 JSON）。当注册了 `fetch` 钩子时，它还会额外作用于 **`module` / `esm`** 类型 remote 的 remote entry 及 manifest 中声明的 CSS——这些资源会通过 `fetch` + blob 加载器加载，从而让自定义的 `headers`（如 `Authorization`）也能应用到这些请求上。
...
`resourceContext` 可用于判断这次 manifest 请求来自
`loadRemote`、`preloadRemote` 还是 shared 初始化。
...
`resourceContext` 标明是哪次请求需要这个 remoteEntry。`cached` 为 `true`
时表示 exports 已经存在；`recovered` 表示加载失败后由 `loadEntryError`
返回了替代 exports。
...
`bridgeHook` 用于观察 React 和 Vue Bridge 的渲染、销毁和路由同步操作。
Bridge 包会把语义化的操作上下文作为第二个参数传入，同时保留第一个
`info` 参数，兼容现有插件。
...
Bridge 生产方或消费方渲染、更新前触发。可以返回对象扩展渲染参数（例如追加
`extraProps`）。
...
Bridge 完成路由同步后触发。React 和 Vue 消费方会报告 host 到 remote 的
`popstate` 同步；Vue 生产方还会报告 memory route 的首次跳转。框架提供返回值时，
可以从 `operation.result` 读取。
