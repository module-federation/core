...
在 {props.brandName || props.name || 'Module Federation'} 中，全局变量主要有 `__FEDERATION__`、`__SHARE__` 和 `__INSTANCES__`，这些变量记录了当前应用的模块联邦信息、共享模块信息和实例信息，可以帮助我们更好地理解和调试模块联邦的运行时行为。
