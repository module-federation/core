...
在 { props.name || 'Module Federation'} 中，全局变量主要有 `__FEDERATION__`、<code>{ props.shareVar || 'SHARE' }</code> 和 <code>{ props.instanceVar || 'INSTANCES' }</code>，这些变量记录了当前应用的模块联邦信息、共享模块信息和实例信息，可以帮助我们更好地理解和调试模块联邦的运行时行为。
...
:::tip
推荐使用 [chrome devtools](/guide/debug/chrome-devtool.html) 查看处理后的 `moduleInfo`。
:::

记录当前应用所有的模块信息，根据此信息可以获取模块的依赖关系。

`moduleInfo` 结构是对象，其中 key 由两部分组成：`{moduleName}(:{moduleVersion})`。
...
在上述的数据中，可以看到 `manifest_host` 和 `app1` 两个模块的共享模块信息。例如我们希望查看 `react` 模块的加载情况，可以按照以下步骤进行分析：

1. `react` 总共只有 `18.3.1` 一个版本。
2. 查看 `__SHARE__["app1:0.1.94"].default.react["18.3.1"]` 可以看到模块 `loaded` 为 true，说明模块已经加载完成。
3. `from` 字段为 `webpack_provider`，说明 `react` 模块是由该模块提供的。
4. 最后查看 `useIn` 字段，可以看到字段中包含了 `webpack_provider` 和 `app1` 两个模块，说明这个版本的 `react` 模块被这两个模块使用了。
