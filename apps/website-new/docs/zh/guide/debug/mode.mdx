...
为了便于排查问题，{props.name || 'Module Federation'} 提供了调试模式，你可以在执行构建时添加 FEDERATION_DEBUG=true 环境变量或者在浏览器执行 `localStorage.setItem('FEDERATION_DEBUG','true')` 来开启 {props.name || 'Module Federation'} 的调试模式。
