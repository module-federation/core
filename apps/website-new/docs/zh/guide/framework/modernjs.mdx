...
在 `modern-app-env.d.ts` 文件增加 `/// <reference types='@module-federation/modern-js/types' />` 以获取类型支持。
...
<Collapse>
```ts
declare function createRemoteSSRReactComponent(
...
import { registerRemotes, createRemoteSSRReactComponent } from '@modern-js/runtime/mf';
...
该函数会收集在服务端渲染的生产者相关的脚本和样式，并返回 script/link 标签。
