...
在消费者中，我们需要通过 `createLazyComponent` 来加载远程组件，并获取数据。
...
除了默认的参数以外，还可以在 `createLazyComponent` 中传递 [dataFetchParams](#datafetchparams) 字段，该字段会被透传给 loader 函数。
...
) : (options: CreateLazyComponentOptions<T, E>) => (props: ComponentType) => React.JSX.Element
```
</Collapse>
...
```tsx
import { getInstance } from '@module-federation/modern-js/runtime';
import { createLazyComponent, wrapNoSSR } from '@module-federation/modern-js/runtime';
