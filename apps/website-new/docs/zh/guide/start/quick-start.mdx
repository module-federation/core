...
本指南将引导你逐步学习使用 Module Federation。我们将构建两个独立的单页面应用程序 (SPA)，它们会使用 Module Federation 共享组件，下文中遇到不熟的名词请查看[名词解释](./glossary)。
...
如果你是在已有项目中接入 Module Federation，请先查看[接入方案](../../integrations/)，根据你的框架或构建工具选择对应的包。
...
如果你想将 Module Federation 集成到已有项目中，请先查看[接入方案](../../integrations/)。那里会告诉你在 Rsbuild、Rspack、Webpack、Modern.js、Next.js、Vite 等项目中应该安装哪个包，以及下一步该看哪篇文档。
