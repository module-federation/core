...
本指南将引导你逐步学习使用 Module Federation。我们将构建两个独立的单页面应用程序 (SPA)，它们会使用 Module Federation 共享组件，下文中遇到不熟的名词请查看[名词解释](/guide/start/glossary.html)。

## 环境要求

使用 Module Federation 构建插件时，请确保 Node.js 版本不低于 20。

```bash
node -v
```

如果你是在已有项目中接入 Module Federation，请先查看[接入方案](/integrations/index.html)，根据你的框架或构建工具选择对应的包。
...
默认创建的消费者项目会消费一个已发布的远端生产者。如果你想消费本地启动的生产者，请在 `module-federation.config.ts` 中修改 `remotes` 配置：
...
[create-module-federation](https://www.npmjs.com/package/create-module-federation) 也支持通过 CLI 参数跳过交互式选择，一键完成创建。例如，将 Modern.js 生产者项目创建到 `my-project` 目录：
...
| provider-rslib-storybook  | 使用 [Rslib](https://lib.rsbuild.dev/) 的生产者，并开启了 Storybook |
| consumer-modern           | 使用 [Modern.js](https://modernjs.dev/) 的消费者             |
| consumer-rsbuild          | 使用 [Rsbuild](https://rsbuild.dev/) 的消费者                |

## 已有项目创建

如果你想将 Module Federation 集成到已有项目中，请先查看[接入方案](/integrations/index.html)。那里会告诉你在 Rsbuild、Rspack、Webpack、Modern.js、Next.js、Vite 等项目中应该安装哪个包，以及下一步该看哪篇文档。
