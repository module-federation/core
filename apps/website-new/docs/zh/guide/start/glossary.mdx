...
不同的构建工具、框架应该选择哪个插件，请查看[接入方案](/integrations)。
...
具体项目应该选择哪个工具和插件，可以查看[接入方案](/integrations)。

## Manifest

Manifest 是生产者在构建时生成的运行时清单，用于描述远程入口、暴露模块、资源、共享依赖和类型信息。消费者可以通过 `mf-manifest.json` 获取这些信息，再加载对应的远程模块。

更多内容可以查看 [Manifest 与 Snapshot](/guide/basic/manifest-snapshot)、[manifest 配置](/configure/manifest) 和 [mf-manifest.json 字段定义](/guide/advanced/manifest-fields)。
