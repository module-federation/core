...
:::tip Runtime API 引用
以下示例默认从 `@module-federation/enhanced/runtime` 引用 Runtime API。Modern.js 项目可以改为从 `@module-federation/modern-js-v3/runtime` 或 `@module-federation/modern-js/runtime` 引用。更多接入方式请参考 [如何引用 Runtime API](/integrations#如何引用-runtime-api)。
:::
