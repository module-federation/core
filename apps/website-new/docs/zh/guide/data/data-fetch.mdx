...
核心 data-fetch API（`cache`、`prefetch`、`DataFetchParams` 等类型及相关工具）位于框架无关的 `@module-federation/data-fetch` 包中。`@module-federation/bridge-react/data-fetch` 与 `@module-federation/bridge-vue3` 为方便使用会重新导出这些 API。React 的 `createLazyComponent` 仍是用于加载组件远程模块的 React UI 适配器（见[懒加载与预取组件](/guide/bridge/react/load-component.html)）。

直接从 `@module-federation/data-fetch` 导入时，请先安装：

```bash
npm install @module-federation/data-fetch
# 或：pnpm add @module-federation/data-fetch
```

若仅从 bridge 包（`@module-federation/bridge-react/data-fetch`、`@module-federation/bridge-vue3`）导入，仍可依赖其重新导出，无需直接安装 `@module-federation/data-fetch`。
