...
当设置为 `true` 时，插件生成的 federation 启动代码不会导入 `@module-federation/runtime-core` 的 snapshot 能力，因此 `snapshotPlugin()` 与 `generatePreloadAssetsPlugin()` 不会被打包，也不会在 Module Federation 实例中初始化。
