...
### disableRemote、disableExpose 和 disableShared

- 类型：`boolean`
- 是否必填：否
- 默认值：`false`

这三个选项会在构建时分别移除 runtime-core 中对应的能力：

- `disableRemote`：移除远程模块的注册、加载、预加载及相关快照处理。仅当当前构建不会消费或动态注册任何远程模块时使用。
- `disableExpose`：移除从已加载远程入口中读取并执行暴露模块的代码。远程入口仍可仅为共享依赖协商而完成初始化。
- `disableShared`：移除共享依赖的注册、选择与加载。运行时会保留最小的空共享作用域，使不使用共享依赖的远程入口仍能初始化。

三个选项可以组合使用。调用已关闭能力对应的接口时会直接报出明确错误。只有在确认当前构建一定不会使用某项能力时，才应开启对应选项。
