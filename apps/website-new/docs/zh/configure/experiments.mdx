...
包还可以接受 `module-federation:target-web`、`module-federation:target-node`、`module-federation:target-worker` 和 `module-federation:target-universal`。本轮 rollout 中，Enhanced 和 Rspack 都不会设置这些条件。
