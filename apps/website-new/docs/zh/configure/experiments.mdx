...
启用 `asyncStartup` 后，所有 {props.brandName || props.name || 'Module Federation'} 入口将默认以异步方式初始化。这意味着：
...
仅用于纯消费者（没有设置 `exposes`）的场景。开启后会自动注入 `@module-federation/inject-external-runtime-core-plugin` 以在运行时提供必要的外部 runtime。
...
- **收益：** 排除这两个插件的代码后，可减小 {props.brandName || props.name || 'Module Federation'} 运行时的整体包体积。
...
### disableRemote 和 disableShared

- 类型：`boolean`
- 是否必填：否
- 默认值：`false`

这两个选项会在构建时分别移除 runtime-core 中对应的能力：

- `disableRemote`：移除完整的远程消费流程，包括注册、入口加载、预加载、读取和执行远程暴露模块及相关快照处理。它不会删除当前生产者由构建工具生成的 `exposes` 或 `container.get`。仅当当前构建不会消费或动态注册任何远程模块时使用。
- `disableShared`：移除共享依赖的注册、选择与加载。运行时会保留最小的空共享作用域，使不使用共享依赖的远程入口仍能初始化。

两个选项可以组合使用。调用已关闭能力对应的接口时会直接报出明确错误。只有在确认当前构建一定不会使用某项能力时，才应开启对应选项。

### target

- 类型：`'web' | 'node'`
- 是否必填：否
- 默认值：从 Webpack 的 `target` 选项推断（通常为 `'web'`）
...
`FEDERATION_OPTIMIZE_*` 和 `ENV_TARGET` 仍然控制默认叶子模块。只有所有受支持的打包器都改用 `module-federation:*` 条件之后，未来的主版本才会删除这些 define。

Package contract 预留了 `module-federation:target-web`、`module-federation:target-node`、`module-federation:target-worker` 和 `module-federation:target-universal`。所选 family 仍声明 `legacy-defines` 时，compiler plugin 不会添加这些 condition。
