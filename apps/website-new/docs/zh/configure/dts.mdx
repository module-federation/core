...
- 示例：[嵌套类型重导出](/guide/basic/type-prompt.html#嵌套类型重导出)
...
- 示例：[第三方包类型抽取](/guide/basic/type-prompt.html#第三方包类型抽取)
...
- 是否必填：否
- 默认值：`undefined`
- 示例：[Federation Runtime API 类型提示](/guide/basic/type-prompt.html#federation-runtime-api-类型提示)
...
- 是否必填：否
- 默认值：`true`
- 使用场景：用于控制 <code>{props.brandName || props.name || 'Module Federation'}</code> 消费（加载）类型行为
...
默认情况下，MF 不会在生产环境（process.env.NODE_ENV === 'production'）去加载类型文件。如果需要在生产环境去加载类型文件，需要设置 `typesOnBuild` 为 `true`。
