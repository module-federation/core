...
- `infer`: 根据 `usedExports` 进行推断，如果提供方的 `usedExports` 符合当前消费方的 `usedExports`，那么就会使用提供方的 `shared`，否则会使用消费方自身的 `shared`，如果都不满足，就使用全量的。
- `server`: 根据服务端下发的 snapshot 来决定是否加载共享依赖。
