...
| `react.enabled` | `boolean` | `false` | 是否尝试自动观测远程 React 函数组件的渲染生命周期。默认关闭。 |
| `react.timeout` | `number` | `5000` | 开启 React 生命周期观测后，组件迟迟没有 mount 时多久记录超时事件。 |
| `react.consumerNames` | `string[]` | `[]` | 只对指定消费者实例开启 React 生命周期观测。为空时不按消费者过滤。 |
| `react.remoteIds` | `string[]` | `[]` | 只对指定远程请求开启 React 生命周期观测，例如 `remote/Button` 或 `./Button`。为空时不按远程请求过滤。 |
...
开启 React 生命周期观测后，插件会自动给远程 React 组件注入 `onMFRemoteLoaded` prop。生产者组件在自己认为业务可用时调用它即可：
...
如果是开发环境或 AI 调试场景，也可以手动开启远程 React 组件生命周期观测：
...
开启后，插件会在 `loadRemote` 成功后尝试识别远程 React 函数组件，并包一层不产生 DOM 的组件。报告里会出现：

- `component:react-render-started`
- `component:react-mounted`
- `component:react-render-timeout`

这个包装还会给远程组件注入 `onMFRemoteLoaded` prop。这个能力只能说明 React 组件已经 mount，不能说明业务数据、图表或 SDK 已经完成。需要表达业务可用时，在生产者组件里调用 `props.onMFRemoteLoaded?.()`。
