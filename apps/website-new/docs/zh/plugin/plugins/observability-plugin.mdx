...
import DivebellObservability from '@docs/zh/_components/divebell-observability.mdx';

## 使用 {props.observabilityToolName || 'Divebell'} 观测 {props.observabilityTargetName || 'Module Federation'}
...
Vite 把构建观测插件放在 `@module-federation/vite` 的 `federation()` 旁边。可以显式传入同一份 Module Federation 配置，也可以让适配器从已解析的 Vite 插件列表读取。运行时观测仍然使用 `ObservabilityPlugin`，通过 `createInstance` 或 `runtimePlugins` 接入，不要把它放到 Vite 构建适配器里。
...
`ObservabilityBuildPlugin` 和 `ObservabilityVitePlugin` 使用同一套参数。Vite 适配器会把 `bundler` 设为 `"vite"`，并在产物写出后从磁盘读取 `mf-manifest.json` / `mf-stats.json`，而不是从 webpack assets 读取。
...
`ObservabilityBuildPlugin(options)` 和 `ObservabilityVitePlugin(options)` 支持以下参数：
...
| `bundler` | `string` | 自动识别（webpack / rspack / vite） | 手动指定 bundler 名称。 |
