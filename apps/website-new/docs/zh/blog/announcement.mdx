...
Module Federation 作为 Webpack 5 推出的重磅功能距离发布至今已经三年有余，Module Federation 作为一种模块共享方案已经在被广泛使用，它受到了大型 Web 项目的欢迎，它让开发大型 Web 应用和团队协同变更的更加高效和简单，但随着 Module Federation 被更加广泛的使用也遇到了新的问题，Module Federation 2.0 针对其中一些问题有了全新的解决方案：

- **增加最佳实践**：Module Federation  上线了[新独立文档站点](https://module-federation.io/)，包括关于 Module Federation 的最佳实践和配置它能够让你更好的使用 Module Federation。
- **Runtime 与构建工具解耦**：新版 Module Federation 将 Runtime 与构建工具解耦，统一各构建工具对于 Module Federation 的实现标准，并让 Module Federation 模块加载更加灵活。
- **类型安全**：新版 Module Federation 构建插件会针对 TypeScript 项目会自动生成和加载类型，在开发模式下会自动同步类型来确保类型安全。
- **调试工具**：新版 Module Federation 提供了 devtool 工具，它让 Module Federation 调试开发变得更加轻松和高效。
- **部署平台**：新版 Module Federation 增加了 mf-manifest.json 文件协议，能够更加容易的打通部署平台来让 Module Federation 版本资源和版本变得更加可控。
