...
我们最近发布了 **[Module Federation 2.0](https://github.com/module-federation/universe)** **v0.1** 版本。
...
- **Rspack**：Rspack 中的 `Module Federation` 基于 `Federation Runtime`，因此能够享受 `Module Federation 2.0` 所有额外的能力，并且由于 `Module Federation` 分治的架构思想，你可以将你的大型项目通过 `Module Federation` 渐进式迁移至 `Rspack`。


### 💡 下一步

目前 `Module Federation 2.0` 仍在快速迭代中，并计划引入更多强大的新特性。
...
`Module Federation 2.0` 的 `0.1` 发布离不开 `Module Federation` 的反馈和建议、 [`Universe`](https://github.com/module-federation/universe) 仓库贡献者做出的努力。
