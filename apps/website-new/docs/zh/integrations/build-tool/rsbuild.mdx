...
帮助用户快速在 **Rsbuild App** 中构建 Module Federation 产物。
...
### 注意
如果需要使用 Module Federation 运行时能力，请安装 [@module-federation/enhanced](/guide/runtime/index.html)
...
使用 `target: 'dual'` 生成 SSR 产物后，可参考 [创建 Modern.js 消费者](/integrations/framework/modernjs/quick-start.html) 创建消费者，并接入对应的 Rslib SSR 生产者进行开发。
