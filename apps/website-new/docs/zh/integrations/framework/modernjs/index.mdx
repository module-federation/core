...
- Modern.js v3：推荐使用 `@module-federation/modern-js-v3`
- Modern.js v2.56.1 及以上：使用 `@module-federation/modern-js`
- 包含服务器端渲染（SSR）
...
参考[数据获取](/guide/data/data-fetch.html)。
...
参考 [React Bridge](/guide/bridge/react/getting-started.html)。

在 Modern.js v3 中，Bridge API 已经被内置在 `@module-federation/modern-js-v3` 插件中，不需要额外安装 `@module-federation/bridge-react` 来使用 Bridge API。

所有 Bridge API 都被放在了 `@module-federation/modern-js-v3/react` 中，你可以从这个路径下引入并使用它们。如果需要显式区分 React 18 或 React 19 下对 `createBridgeComponent` 的调用，Modern.js v3 插件的导出路径略有不同：
...
::: danger
此 API 已被废弃，请使用[createLazyComponent](/guide/bridge/react/load-component.html#什么是-createlazycomponent) 。
:::
...
迁移方式同 [createRemoteComponent](#createremotecomponent)。
