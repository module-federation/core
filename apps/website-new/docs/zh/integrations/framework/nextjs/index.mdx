...
Next.js 会直接从 `mf-manifest.json` 派生旧版 `flushChunks()` 使用的 expose 资源字段，无需额外遍历 Webpack stats，因此滚动升级期间旧版本应用仍可继续工作。读取其他旧版 stats 字段的自定义工具应迁移到 `mf-manifest.json`；设置 `manifest: false` 可保留完整的旧版输出。
