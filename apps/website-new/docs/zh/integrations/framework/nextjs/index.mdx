...
new NextFederationPlugin({
  ...
  shared: {},
  extraOptions: {
    debug: boolean, // 默认为 `false`
    exposePages: boolean, // 默认为 `false`
    enableImageLoaderFix: boolean, // 默认为 `false`
    enableUrlLoaderFix: boolean, // 默认为 `false`
    skipSharingNextInternals: boolean, // 默认为 `false`
    skipFederatedStats: boolean // 默认为 `false`
  },
});
```
...
- `skipFederatedStats` – 跳过 `federated-stats.json` 的生成。默认情况下，Next.js 会直接从 `mf-manifest.json` 派生旧版 `flushChunks()` 使用的 expose 资源字段，无需额外遍历 Webpack stats，因此滚动升级期间旧版本应用仍可继续工作。读取其他旧版 stats 字段的自定义工具应迁移到 `mf-manifest.json`；如果暂时仍需要完整旧版输出，请设置 `manifest: false`。
