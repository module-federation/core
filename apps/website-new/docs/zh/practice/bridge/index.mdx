...
`MF bridge` 是 `Module Federation` 提供的工具函数，用于帮助用户通过 `Module Federation` 来加载应用级别模块。它能自动提供 `provider` 函数所需的应用生命周期函数 `render` 和 `destroy`，并实现框架不同版本的兼容。此外，它还能让应用间的路由正常协同工作。
