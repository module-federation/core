...
- Step2: 在 rsbuild 配置中，我们需要将 `export-app.tsx` 作为应用类型模块导出
...
- Step1: 在 rsbuild 配置中，我们需要注册远程模块，这点与其它 Module Federation 配置无异。
...
2. 此外，createRemoteComponent 会自动处理模块加载、错误处理、loading、路由 等逻辑，
开发者只需要关注如何使用远程组件即可。
