...
```tsx
const Remote1App = createRemoteComponent({ loader: () => import('remote1/export-app') });
```

* `options`
  * `loader`
    * type: `() => Promise<Module>`
    * Purpose: Used to load remote modules, e.g., `import('remote1/export-app')` or `loadRemote('remote1/export-app')`
  * `export`
    * type: `string`
...
const Remote2 = bridge.createRemoteComponent({ loader: () => import('remote1/export-app'), rootAttrs: {class: 'root-element-class'} });
