...
下表展示了 `@module-federation/bridge-react` 包自身的体积差异（未压缩，gzip 后约节省 0.5-1 KB）：

| 配置 | bridge-react 包体积 | 包含 react-router-dom |
|------|-------------------|----------------------|
| `enableBridgeRouter: true` (默认) | ~9 KB | ✅ 是 |
| `enableBridgeRouter: false` | ~6 KB | ❌ 否 |
| **节省** | **~3 KB** (约 33%) | - |
...
使用 `enableBridgeRouter: false` 构建后，验证 bundle 中不包含 react-router：

```bash
# 构建应用
npm run build

# 检查 bundle 是否包含 react-router
