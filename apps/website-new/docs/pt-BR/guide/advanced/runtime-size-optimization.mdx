...
Esses defines `FEDERATION_*` ficam até uma major futura. Um bundler que define uma package condition como `module-federation:no-remote` já pode deixar o módulo correspondente fora do grafo. Os módulos padrão ainda leem esses defines.
