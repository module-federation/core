...
| Navegador integrado a um aplicativo de IA que aceita extensões Chrome, como o navegador do Codex | Browser (`dist/browser`) | Popup compacto pelo ícone da extensão               |

As duas variantes oferecem as mesmas funcionalidades: **proxy e troca de versões de módulos, informações de módulos, grafo de dependências, dependências compartilhadas, React HMR / Fast Refresh, Loading Trace e exportação de relatórios**, e ambas suportam WebMCP. A descoberta e a chamada das ferramentas WebMCP foram verificadas no navegador integrado do Codex.
...
2. Extraia o ZIP e carregue a pasta extraída no gerenciador de extensões do navegador integrado.
