...
`@module-federation/bridge-vue3` fornece uma `bridge` utility função for Vue V3 aplicações. O fornecido `createBridgeComponent` pode ser usado para exporte aplicação-level módulos, e `createRemoteAppComponent` pode ser usado para carregar aplicação-level módulos.
...
* `options`
  * `loader`
    * type: `() => Promise<Module>`
    * Purpose: Usado para carregar módulos remotos, e.g., `loadRemote('remote1/export-app')` ou `import('remote1/export-app')`
  * `export`
    * tipo: `string`
    * Purpose: Usado para specify módulo exporte
  * `asyncComponentOptions`
    * type: `Omit<AsyncComponentOptions, 'loader'>`
    * Purpose: Parameters esse vai ser passed para defineAsyncComponent, except para o loader parameter
  * `rootAttrs`
    * type: `Record<string, unknown>`
    * Purpose: Attributes esse vai ser bound para o root container onde o remote Vue aplicação vai ser mounted
```tsx
// remote
...
* ReturnType
  * tipo: `VueComponent`
  * Purpose: Usado para render módulo remoto componentes
...
* `bridgeInfo`
  * tipo: `{ rootComponent: VueComponent; appOptions?: (params: AddOptionsFnParams) => ({ router?: Router }) }`
  * Purpose: Usado para pass o root componente
* ReturnType
  * tipo: `() => { render: (info: RenderFnParams) => void; destroy: (info: { dom: HTMLElement}) => void; }`
