...
function getGitDiffPatch(filePath) {
  try {
    const baseBranch = execSync('git symbolic-ref refs/remotes/origin/HEAD | sed "s@^refs/remotes/origin/@@g" || echo main').toString().trim();
    const patch = execSync(`git diff ${baseBranch} -- "${filePath}"`).toString();
    return patch;
  } catch (error) {
...
function generateRandomFilename() {
  const adjectives = [
    'quick', 'lazy', 'sleepy', 'noisy', 'hungry', 'brave', 'calm', 'eager', 'gentle', 'happy'
  ];
  const animals = [
    'fox', 'dog', 'cat', 'mouse', 'owl', 'tiger', 'lion', 'bear', 'wolf', 'eagle'
  ];
