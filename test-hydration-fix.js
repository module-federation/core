...
const SHOP_PAGE_PATH = path.join(
  __dirname,
...
function checkShopPageContent() {
  try {
    ...
    if (content.includes('HYDRATION FIX TEST 123')) {
      console.log(
        '✅ Shop page contains user modification: "HYDRATION FIX TEST 123"',
      );
      return true;
    } else {
...
async function runHydrationTests() {
  try {
    ...
    if (!checkShopPageContent()) {
      console.log(
        '📝 Note: Shop page will be modified during tests to include user changes',
      );
    }
